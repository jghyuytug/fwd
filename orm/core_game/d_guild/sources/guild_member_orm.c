...
int GuildMember_GetByCharacNo(DBConnectionManager* manager, int charac_no, GuildMember* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT guild_id, m_id, server_id, charac_no, charac_name, memo, grade, job, grow_type, lev, age, born_year, sex, apply_time, member_time, member_flag, bbs_cnt, last_visit_time, secede_type, secede_time, member_point, member_point_prev, last_play_time, nick_name FROM guild_member WHERE charac_no = %d AND member_flag = 1 LIMIT 1",
        charac_no);
...
int GuildMember_GetByGuild(DBConnectionManager* manager, int guild_id, GuildMember* members, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT guild_id, m_id, server_id, charac_no, charac_name, memo, grade, job, grow_type, lev, age, born_year, sex, apply_time, member_time, member_flag, bbs_cnt, last_visit_time, secede_type, secede_time, member_point, member_point_prev, last_play_time, nick_name FROM guild_member WHERE guild_id = %d AND member_flag = 1 ORDER BY grade ASC, lev DESC LIMIT %d",
        guild_id, max_count);
    ...
    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        ...
        members[count].m_id = row[1] ? atoi(row[1]) : 0;
        members[count].server_id = row[2] ? atoi(row[2]) : 0;
        members[count].charac_no = row[3] ? atoi(row[3]) : 0;
        if (row[4]) strncpy(members[count].charac_name, row[4], sizeof(members[count].charac_name) - 1);
        if (row[5]) strncpy(members[count].memo, row[5], sizeof(members[count].memo) - 1);
        members[count].grade = row[6] ? atoi(row[6]) : 0;
        members[count].job = row[7] ? atoi(row[7]) : 0;
        members[count].grow_type = row[8] ? atoi(row[8]) : 0;
        members[count].lev = row[9] ? atoi(row[9]) : 0;
        members[count].age = row[10] ? atoi(row[10]) : 0;
        if (row[11]) strncpy(members[count].born_year, row[11], sizeof(members[count].born_year) - 1);
        if (row[12]) strncpy(members[count].sex, row[12], sizeof(members[count].sex) - 1);
        if (row[13]) strncpy(members[count].apply_time, row[13], sizeof(members[count].apply_time) - 1);
        if (row[14]) strncpy(members[count].member_time, row[14], sizeof(members[count].member_time) - 1);
        members[count].member_flag = row[15] ? atoi(row[15]) : 0;
        members[count].bbs_cnt = row[16] ? atoi(row[16]) : 0;
        if (row[17]) strncpy(members[count].last_visit_time, row[17], sizeof(members[count].last_visit_time) - 1);
        members[count].secede_type = row[18] ? atoi(row[18]) : 0;
        if (row[19]) strncpy(members[count].secede_time, row[19], sizeof(members[count].secede_time) - 1);
        members[count].member_point = row[20] ? atoi(row[20]) : 0;
        members[count].member_point_prev = row[21] ? atoi(row[21]) : 0;
        if (row[22]) strncpy(members[count].last_play_time, row[22], sizeof(members[count].last_play_time) - 1);
        if (row[23]) strncpy(members[count].nick_name, row[23], sizeof(members[count].nick_name) - 1);
        count++;
    }
...
int GuildMember_ClearGuild(DBConnectionManager* manager, int guild_id, int flag) {
    ...
    snprintf(query, sizeof(query),
             "UPDATE guild_member SET member_flag = %d WHERE guild_id = %d",
             flag, guild_id);
