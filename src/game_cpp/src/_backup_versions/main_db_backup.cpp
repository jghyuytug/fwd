...
static void calculate_combat_stats(ClientSession* s) {
    ...
    /* Equipment bonuses (simplified: item_id contributes to stats) */
    for (int i = 0; i < MAX_EQUIP_SLOTS; i++) {
        uint32_t item_id = s->equipment.equipment[i].it_id;
        if (item_id == 0) continue;

        /* Simple formula: higher item_id = better stats */
        uint32_t item_power = (item_id % 1000) / 10;
        uint8_t enhance = s->equipment.equipment[i].stat;

        switch (i) {
            case EQUIP_SLOT_WEAPON:
                stats->phys_attack += item_power * 10 + enhance * 20;
                stats->mag_attack += item_power * 8 + enhance * 15;
                break;
            case EQUIP_SLOT_CHEST:
            case EQUIP_SLOT_PANTS:
                stats->phys_def += item_power * 5 + enhance * 10;
                stats->mag_def += item_power * 4 + enhance * 8;
                stats->max_hp += item_power * 20;
                break;
            case EQUIP_SLOT_HELMET:
            case EQUIP_SLOT_SHOULDER:
            case EQUIP_SLOT_SHOES:
                stats->phys_def += item_power * 3 + enhance * 6;
                stats->mag_def += item_power * 2 + enhance * 4;
                break;
            case EQUIP_SLOT_BELT:
            case EQUIP_SLOT_BRACELET:
            case EQUIP_SLOT_NECKLACE:
            case EQUIP_SLOT_RING:
                stats->str += item_power / 2;
                stats->intel += item_power / 2;
                break;
        }
    }
...
static int handle_repair_equipment(ClientSession* s, const uint8_t* data, int len) {
    ...
    uint16_t slot;
    memcpy(&slot, data, 2);
    printf("[GAME] REPAIR_EQUIPMENT: slot=%u\n", slot);

    /* TODO: Deduct gold, update durability in database */
    uint8_t response[8];
    memset(response, 0, 8);
    /* response[0-3] = result, response[4-7] = remaining gold */
    return send_response(s, 25, response, 8);
}
...
static void dungeon_spawn_monsters(DungeonInstance* d, int room) {
    ...
    printf("[DUNGEON] Spawned %d monsters in room %d\n", d->monster_count, room);
}
...
static int handle_dungeon_enter(ClientSession* s, const uint8_t* data, int len) {
    printf("[GAME] DUNGEON_ENTER from fd=%d\n", s->fd);
    ...
    d->current_room = 0;
    d->total_rooms = 3 + difficulty;  /* 3-7 rooms based on difficulty */
    d->active = 1;
    d->start_time = (uint32_t)time(NULL);
    d->time_limit = 600;  /* 10 minutes */
    d->monsters_killed = 0;

    /* Spawn first room monsters */
    ...
    for (int i = 0; i < d->monster_count && offset < 120; i++) {
        MonsterInfo* m = &d->monsters[i];
    ...
    printf("[DUNGEON] Entered dungeon %u room 0/%d with %d monsters\n",
           dungeon_id, d->total_rooms, d->monster_count);
...
static int handle_dungeon_exit(ClientSession* s, const uint8_t* data, int len) {
    ...
    printf("[DUNGEON] Exited dungeon (killed %d monsters)\n", s->dungeon.monsters_killed);

    uint8_t response[8];
    memset(response, 0, sizeof(response));
    uint32_t result = 0;
    memcpy(response, &result, 4);
    memcpy(response + 4, &s->dungeon.monsters_killed, 4);
...
static int handle_dungeon_attack(ClientSession* s, const uint8_t* data, int len) {
    ...
    if (damage >= target->hp) {
        ...
        target->hp -= damage;
        printf("[DUNGEON] Monster %u took %u damage, HP: %u/%u\n",
               monster_id, damage, target->hp, target->max_hp);
    }
