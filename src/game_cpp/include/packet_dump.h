...
/* 全局状态 */
static FILE* g_packet_dump_file = NULL;
static int g_packet_dump_enabled = 0;
static uint32_t g_packet_dump_count = 0;
...
static inline void packet_dump_write(const char* direction, const uint8_t* data,
    ...
    if (len >= 13) {
        ...
        memcpy(&msg_no, data + 5, 2);

        fprintf(g_packet_dump_file,
                "[Legacy 13-byte]         class=%d len=%u msg_no=%d(0x%04X)%s\n",
                classification, length, msg_no, msg_no,
