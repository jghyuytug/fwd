...
int MemberPunishInfoHistory2012_Add(DBConnectionManager* manager, const MemberPunishInfoHistory2012* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO member_punish_info_history_2012 (m_id, punish_type, occ_time, punish_value, apply_flag, start_time, end_time, admin_id, reason, is_kicked, first_ssn, second_ssn) "
...
int MemberPunishInfoHistory2012_Get(DBConnectionManager* manager, int no, MemberPunishInfoHistory2012* record) {
    char query[MAX_QUERY_LEN];
    DBQueryResult result;
    char* row[13];
    ...
    snprintf(query, sizeof(query),
        "SELECT no, m_id, punish_type, occ_time, punish_value, apply_flag, start_time, end_time, admin_id, reason, is_kicked, first_ssn, second_ssn FROM member_punish_info_history_2012 WHERE no = %d",
        no);
...
int MemberPunishInfoHistory2012_GetAll(DBConnectionManager* manager, MemberPunishInfoHistory2012* records, int max_count, int* actual_count) {
    char query[MAX_QUERY_LEN];
    DBQueryResult result;
    char* row[13];
    int count = 0;
    ...
    snprintf(query, sizeof(query),
        "SELECT no, m_id, punish_type, occ_time, punish_value, apply_flag, start_time, end_time, admin_id, reason, is_kicked, first_ssn, second_ssn FROM member_punish_info_history_2012");
