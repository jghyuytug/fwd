#include "auction_history_201606_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int AuctionHistory201606_Add(DBConnectionManager* manager, const AuctionHistory201606* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO auction_history_201606 (auction_id, start_time, occ_time, event_type, owner_id, buyer_id, price, seal_flag, item_id, add_info, upgrade, amplify_option, amplify_value, seal_cnt, endurance, extend_info, owner_postal_id, buyer_postal_id, unit_price) "
...
int AuctionHistory201606_Get(DBConnectionManager* manager, long long auction_id, AuctionHistory201606* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT auction_id, start_time, occ_time, event_type, owner_id, buyer_id, price, seal_flag, item_id, add_info, upgrade, amplify_option, amplify_value, seal_cnt, endurance, extend_info, owner_postal_id, buyer_postal_id, unit_price FROM auction_history_201606 WHERE auction_id = %d",
        auction_id);
    ...
    memset(record, 0, sizeof(AuctionHistory201606));
...
int AuctionHistory201606_GetAll(DBConnectionManager* manager, AuctionHistory201606* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT auction_id, start_time, occ_time, event_type, owner_id, buyer_id, price, seal_flag, item_id, add_info, upgrade, amplify_option, amplify_value, seal_cnt, endurance, extend_info, owner_postal_id, buyer_postal_id, unit_price FROM auction_history_201606");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_AUCTION_CERA, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(AuctionHistory201606));
