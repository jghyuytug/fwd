#include "auction_history_buyer_201603_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int AuctionHistoryBuyer201603_Add(DBConnectionManager* manager, const AuctionHistoryBuyer201603* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO auction_history_buyer_201603 (auction_id, occ_time, pre_buyer_id, buyer_id, pre_price, price, pre_buyer_postal_id, commission) "
...
int AuctionHistoryBuyer201603_GetAll(DBConnectionManager* manager, AuctionHistoryBuyer201603* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT auction_id, occ_time, pre_buyer_id, buyer_id, pre_price, price, pre_buyer_postal_id, commission FROM auction_history_buyer_201603");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_AUCTION_CERA, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(AuctionHistoryBuyer201603));
