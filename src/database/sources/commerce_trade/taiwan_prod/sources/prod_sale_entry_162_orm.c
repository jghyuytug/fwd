#include "prod_sale_entry_162_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int ProdSaleEntry162_Get(DBConnectionManager* manager, const char* agency_no, ProdSaleEntry162* record) {
    ...
    memset(record, 0, sizeof(ProdSaleEntry162));
...
int ProdSaleEntry162_GetAll(DBConnectionManager* manager, ProdSaleEntry162* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT agency_no, m_id, charac_no, job, grow_type, level, ipg_no, price, buy_type, gift_server_info, gift_get_charac_no, sale_time, charge_price, free_price, count_no, each_price, price_type, seal_flag, slot_part_type, slot_part_code, buy_method_type, prod_part_id, bonus_ipg_no, user_id, server_id, prize_flag, new_flag, result_code, result_message, user_ip, tran_id FROM prod_sale_entry_162");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_PROD, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(ProdSaleEntry162));
