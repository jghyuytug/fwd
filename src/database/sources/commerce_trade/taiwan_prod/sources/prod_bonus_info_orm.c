...
int ProdBonusInfo_Add(DBConnectionManager* manager, const ProdBonusInfo* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO prod_bonus_info (ipg_no, bonus_ipg_no, bonus_rate) "
        "VALUES (%d, %d, %d)",
        record->ipg_no, record->bonus_ipg_no, record->bonus_rate);
...
int ProdBonusInfo_Get(DBConnectionManager* manager, int ipg_no, int bonus_ipg_no, ProdBonusInfo* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT ipg_no, bonus_ipg_no, bonus_rate FROM prod_bonus_info WHERE ipg_no = %d AND bonus_ipg_no = %d",
        ipg_no, bonus_ipg_no);
    ...
    memset(record, 0, sizeof(ProdBonusInfo));
...
int ProdBonusInfo_GetAll(DBConnectionManager* manager, ProdBonusInfo* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT ipg_no, bonus_ipg_no, bonus_rate FROM prod_bonus_info");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_PROD, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(ProdBonusInfo));
