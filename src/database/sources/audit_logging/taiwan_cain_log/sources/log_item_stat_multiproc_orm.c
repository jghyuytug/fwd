#include "log_item_stat_multiproc_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int LogItemStatMultiproc_Add(DBConnectionManager* manager, const LogItemStatMultiproc* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO log_item_stat_multiproc (cur_date, server_info, process_id, item_id, item_cnt) "
        "VALUES ('%s', %d, %d, %d, %d)",
...
int LogItemStatMultiproc_Get(DBConnectionManager* manager, const char* cur_date, signed char server_info, signed char process_id, int item_id, LogItemStatMultiproc* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT cur_date, server_info, process_id, item_id, item_cnt FROM log_item_stat_multiproc WHERE cur_date = '%s' AND server_info = %d AND process_id = %d AND item_id = %d",
    ...
    memset(record, 0, sizeof(LogItemStatMultiproc));
...
int LogItemStatMultiproc_GetAll(DBConnectionManager* manager, LogItemStatMultiproc* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT cur_date, server_info, process_id, item_id, item_cnt FROM log_item_stat_multiproc");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_CAIN_LOG, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(LogItemStatMultiproc));
