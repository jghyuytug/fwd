#include "log_item_upgrade_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int LogItemUpgrade_Add(DBConnectionManager* manager, const LogItemUpgrade* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO log_item_upgrade (cur_date, server_info, process_id, item_id, item_cnt, upgrade, amplify_item_cnt, rarity, level) "
...
int LogItemUpgrade_Get(DBConnectionManager* manager, const char* cur_date, signed char server_info, signed char process_id, int item_id, LogItemUpgrade* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT cur_date, server_info, process_id, item_id, item_cnt, upgrade, amplify_item_cnt, rarity, level FROM log_item_upgrade WHERE cur_date = '%s' AND server_info = %d AND process_id = %d AND item_id = %d",
    ...
    memset(record, 0, sizeof(LogItemUpgrade));
...
int LogItemUpgrade_GetAll(DBConnectionManager* manager, LogItemUpgrade* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT cur_date, server_info, process_id, item_id, item_cnt, upgrade, amplify_item_cnt, rarity, level FROM log_item_upgrade");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_CAIN_LOG, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(LogItemUpgrade));
