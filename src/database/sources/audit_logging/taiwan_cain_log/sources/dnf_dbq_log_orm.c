...
int DnfDbqLog_Add(DBConnectionManager* manager, const DnfDbqLog* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO dnf_dbq_log (occ_time, channel_no, dbq) "
        "VALUES ('%s', %d, %d)",
        record->occ_time, record->channel_no, record->dbq);
...
int DnfDbqLog_Get(DBConnectionManager* manager, const char* occ_time, signed char channel_no, DnfDbqLog* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT occ_time, channel_no, dbq FROM dnf_dbq_log WHERE occ_time = '%s' AND channel_no = %d",
        occ_time, channel_no);
