#include "rdar0_search_content_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int Rdar0SearchContent_Add(DBConnectionManager* manager, const Rdar0SearchContent* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO rdar0_search_content (no, category, class, word) "
        "VALUES (%d, %d, %d, '%s')",
...
int Rdar0SearchContent_GetAll(DBConnectionManager* manager, Rdar0SearchContent* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, category, class, word FROM rdar0_search_content");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(Rdar0SearchContent));
