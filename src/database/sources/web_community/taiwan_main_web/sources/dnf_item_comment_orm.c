#include "dnf_item_comment_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int DnfItemComment_Add(DBConnectionManager* manager, const DnfItemComment* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO dnf_item_comment (item_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day, recomm_score) "
        "VALUES (%d, '%s', %d, '%s', %d, %d, %d, '%s', %d, '%s', '%s', %d, %d)",
        record->item_no, record->m_nickname, record->m_id, record->m_user_id, record->m_sex, record->server_id, record->charac_no, record->charac_name, record->job, record->comment_text, record->ip, record->create_day, record->recomm_score);
...
int DnfItemComment_Get(DBConnectionManager* manager, int no, DnfItemComment* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, item_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day, recomm_score FROM dnf_item_comment WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(DnfItemComment));
...
int DnfItemComment_GetAll(DBConnectionManager* manager, DnfItemComment* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, item_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day, recomm_score FROM dnf_item_comment");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(DnfItemComment));
