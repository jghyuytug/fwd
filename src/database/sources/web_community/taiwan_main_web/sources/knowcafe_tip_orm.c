...
int KnowcafeTip_Add(DBConnectionManager* manager, const KnowcafeTip* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO knowcafe_tip (notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, adorn, adorn_color1, adorn_color2, file, depth, sequence) "
...
int KnowcafeTip_Get(DBConnectionManager* manager, int no, KnowcafeTip* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, adorn, adorn_color1, adorn_color2, file, depth, sequence FROM knowcafe_tip WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(KnowcafeTip));
...
int KnowcafeTip_GetAll(DBConnectionManager* manager, KnowcafeTip* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, adorn, adorn_color1, adorn_color2, file, depth, sequence FROM knowcafe_tip");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(KnowcafeTip));
