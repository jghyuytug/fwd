...
int Penpic3Comment_Add(DBConnectionManager* manager, const Penpic3Comment* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO penpic3_comment (content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, point, ip, create_day) "
...
int Penpic3Comment_Get(DBConnectionManager* manager, int no, Penpic3Comment* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, point, ip, create_day FROM penpic3_comment WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(Penpic3Comment));
...
int Penpic3Comment_GetAll(DBConnectionManager* manager, Penpic3Comment* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, point, ip, create_day FROM penpic3_comment");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(Penpic3Comment));
