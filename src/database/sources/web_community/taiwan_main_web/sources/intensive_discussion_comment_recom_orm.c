#include "intensive_discussion_comment_recom_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int IntensiveDiscussionCommentRecom_Add(DBConnectionManager* manager, const IntensiveDiscussionCommentRecom* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO intensive_discussion_comment_recom (comment_no, m_id, content_no) "
        "VALUES (%d, %d, %d)",
...
int IntensiveDiscussionCommentRecom_Get(DBConnectionManager* manager, int no, IntensiveDiscussionCommentRecom* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, comment_no, m_id, content_no FROM intensive_discussion_comment_recom WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(IntensiveDiscussionCommentRecom));
...
int IntensiveDiscussionCommentRecom_GetAll(DBConnectionManager* manager, IntensiveDiscussionCommentRecom* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, comment_no, m_id, content_no FROM intensive_discussion_comment_recom");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(IntensiveDiscussionCommentRecom));
