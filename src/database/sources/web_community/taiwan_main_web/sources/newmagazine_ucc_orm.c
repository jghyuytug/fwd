...
int NewmagazineUcc_Add(DBConnectionManager* manager, const NewmagazineUcc* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO newmagazine_ucc (degree, category, seq, title, reg_date, hists, m_id, user_id, server_id, charac_no, charac_name, mod_date, point, point_cnt, search_word, thumbnail_url, writer, summary, open_flag, open_date) "
        "VALUES (%d, %d, %d, '%s', %d, %d, %d, '%s', %d, %d, '%s', %d, %d, %d, '%s', '%s', '%s', '%s', %d, %d)",
...
int NewmagazineUcc_Get(DBConnectionManager* manager, int mz_id, NewmagazineUcc* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT mz_id, degree, category, seq, title, reg_date, hists, m_id, user_id, server_id, charac_no, charac_name, mod_date, point, point_cnt, search_word, thumbnail_url, writer, summary, open_flag, open_date FROM newmagazine_ucc WHERE mz_id = %d",
        mz_id);
    ...
    memset(record, 0, sizeof(NewmagazineUcc));
...
int NewmagazineUcc_GetAll(DBConnectionManager* manager, NewmagazineUcc* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT mz_id, degree, category, seq, title, reg_date, hists, m_id, user_id, server_id, charac_no, charac_name, mod_date, point, point_cnt, search_word, thumbnail_url, writer, summary, open_flag, open_date FROM newmagazine_ucc");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(NewmagazineUcc));
