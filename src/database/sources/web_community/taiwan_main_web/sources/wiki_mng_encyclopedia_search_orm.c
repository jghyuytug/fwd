#include "wiki_mng_encyclopedia_search_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int WikiMngEncyclopediaSearch_Add(DBConnectionManager* manager, const WikiMngEncyclopediaSearch* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO wiki_mng_encyclopedia_search (id, category, class, word) "
        "VALUES (%d, %d, %d, '%s')",
        record->id, record->category, record->class, record->word);
...
int WikiMngEncyclopediaSearch_GetAll(DBConnectionManager* manager, WikiMngEncyclopediaSearch* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT id, category, class, word FROM wiki_mng_encyclopedia_search");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(WikiMngEncyclopediaSearch));
        records[count].id = row[0] ? atoi(row[0]) : 0;
        records[count].category = row[1] ? atoi(row[1]) : 0;
        records[count].class = row[2] ? atoi(row[2]) : 0;
