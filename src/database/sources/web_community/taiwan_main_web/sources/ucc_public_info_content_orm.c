#include "ucc_public_info_content_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int UccPublicInfoContent_Add(DBConnectionManager* manager, const UccPublicInfoContent* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO ucc_public_info_content (no, content, content_type, ip, ring, sms) "
        "VALUES (%d, '%s', %d, '%s', %d, %d)",
...
int UccPublicInfoContent_Get(DBConnectionManager* manager, int no, UccPublicInfoContent* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM ucc_public_info_content WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(UccPublicInfoContent));
...
int UccPublicInfoContent_GetAll(DBConnectionManager* manager, UccPublicInfoContent* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM ucc_public_info_content");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(UccPublicInfoContent));
