#include "knowcafe_job_file_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int KnowcafeJobFile_Add(DBConnectionManager* manager, const KnowcafeJobFile* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO knowcafe_job_file (content_no, file_url) "
        "VALUES (%d, '%s')",
...
int KnowcafeJobFile_Get(DBConnectionManager* manager, int no, KnowcafeJobFile* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, file_url FROM knowcafe_job_file WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(KnowcafeJobFile));
...
int KnowcafeJobFile_GetAll(DBConnectionManager* manager, KnowcafeJobFile* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, file_url FROM knowcafe_job_file");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(KnowcafeJobFile));
