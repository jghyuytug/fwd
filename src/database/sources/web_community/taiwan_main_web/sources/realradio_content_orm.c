#include "realradio_content_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int RealradioContent_Add(DBConnectionManager* manager, const RealradioContent* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO realradio_content (no, content, content_type, ip, ring, sms) "
        "VALUES (%d, '%s', %d, '%s', %d, %d)",
...
int RealradioContent_Get(DBConnectionManager* manager, int no, RealradioContent* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM realradio_content WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(RealradioContent));
...
int RealradioContent_GetAll(DBConnectionManager* manager, RealradioContent* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM realradio_content");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(RealradioContent));
