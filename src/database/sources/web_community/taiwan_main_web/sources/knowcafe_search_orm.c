...
int KnowcafeSearch_Add(DBConnectionManager* manager, const KnowcafeSearch* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO knowcafe_search (knowcafe_type, no, category, class, word) "
        "VALUES (%d, %d, %d, %d, '%s')",
        record->knowcafe_type, record->no, record->category, record->class, record->word);
...
int KnowcafeSearch_GetAll(DBConnectionManager* manager, KnowcafeSearch* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT knowcafe_type, no, category, class, word FROM knowcafe_search");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(KnowcafeSearch));
