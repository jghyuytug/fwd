#include "tcg_public_info_content_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int TcgPublicInfoContent_Add(DBConnectionManager* manager, const TcgPublicInfoContent* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO tcg_public_info_content (no, content, content_type, ip, ring, sms) "
        "VALUES (%d, '%s', %d, '%s', %d, %d)",
...
int TcgPublicInfoContent_Get(DBConnectionManager* manager, int no, TcgPublicInfoContent* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM tcg_public_info_content WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(TcgPublicInfoContent));
...
int TcgPublicInfoContent_GetAll(DBConnectionManager* manager, TcgPublicInfoContent* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM tcg_public_info_content");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(TcgPublicInfoContent));
