#include "penpic_valuation_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int PenpicValuation_Add(DBConnectionManager* manager, const PenpicValuation* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO penpic_valuation (no, penpic_desc, penpic_examine) "
        "VALUES (%d, '%s', '%s')",
...
int PenpicValuation_Get(DBConnectionManager* manager, int no, PenpicValuation* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, penpic_desc, penpic_examine FROM penpic_valuation WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(PenpicValuation));
...
int PenpicValuation_GetAll(DBConnectionManager* manager, PenpicValuation* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, penpic_desc, penpic_examine FROM penpic_valuation");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(PenpicValuation));
