#include "intensive_discussion_comment_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int IntensiveDiscussionComment_Add(DBConnectionManager* manager, const IntensiveDiscussionComment* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO intensive_discussion_comment (content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day, recom, opinion_mode, del_flag, comment_cnt) "
...
int IntensiveDiscussionComment_Get(DBConnectionManager* manager, int no, IntensiveDiscussionComment* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day, recom, opinion_mode, del_flag, comment_cnt FROM intensive_discussion_comment WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(IntensiveDiscussionComment));
...
int IntensiveDiscussionComment_GetAll(DBConnectionManager* manager, IntensiveDiscussionComment* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day, recom, opinion_mode, del_flag, comment_cnt FROM intensive_discussion_comment");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(IntensiveDiscussionComment));
