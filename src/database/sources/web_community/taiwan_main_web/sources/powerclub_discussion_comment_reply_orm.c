#include "powerclub_discussion_comment_reply_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int PowerclubDiscussionCommentReply_Add(DBConnectionManager* manager, const PowerclubDiscussionCommentReply* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO powerclub_discussion_comment_reply (no, m_nickname, m_id, m_user_id, server_id, charac_no, charac_name, job, comment_text, create_day, depth, sequence, del_flag, content_no) "
        "VALUES (%d, '%s', %d, '%s', %d, %d, '%s', %d, '%s', %d, %d, %f, %d, %d)",
...
int PowerclubDiscussionCommentReply_Get(DBConnectionManager* manager, int rno, PowerclubDiscussionCommentReply* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT rno, no, m_nickname, m_id, m_user_id, server_id, charac_no, charac_name, job, comment_text, create_day, depth, sequence, del_flag, content_no FROM powerclub_discussion_comment_reply WHERE rno = %d",
        rno);
    ...
    memset(record, 0, sizeof(PowerclubDiscussionCommentReply));
    ...
    record->sequence = row[12] ? atof(row[12]) : 0;
...
int PowerclubDiscussionCommentReply_GetAll(DBConnectionManager* manager, PowerclubDiscussionCommentReply* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT rno, no, m_nickname, m_id, m_user_id, server_id, charac_no, charac_name, job, comment_text, create_day, depth, sequence, del_flag, content_no FROM powerclub_discussion_comment_reply");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(PowerclubDiscussionCommentReply));
        ...
        records[count].sequence = row[12] ? atof(row[12]) : 0;
