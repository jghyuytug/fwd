#include "penpic3_valuation_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int Penpic3Valuation_Add(DBConnectionManager* manager, const Penpic3Valuation* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO penpic3_valuation (no, penpic_desc, penpic_examine) "
        "VALUES (%d, '%s', '%s')",
...
int Penpic3Valuation_Get(DBConnectionManager* manager, int no, Penpic3Valuation* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, penpic_desc, penpic_examine FROM penpic3_valuation WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(Penpic3Valuation));
...
int Penpic3Valuation_GetAll(DBConnectionManager* manager, Penpic3Valuation* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, penpic_desc, penpic_examine FROM penpic3_valuation");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(Penpic3Valuation));
