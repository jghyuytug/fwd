#include "intensive_discussion_file_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int IntensiveDiscussionFile_Add(DBConnectionManager* manager, const IntensiveDiscussionFile* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO intensive_discussion_file (content_no, file_url) "
        "VALUES (%d, '%s')",
...
int IntensiveDiscussionFile_Get(DBConnectionManager* manager, int no, IntensiveDiscussionFile* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, file_url FROM intensive_discussion_file WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(IntensiveDiscussionFile));
...
int IntensiveDiscussionFile_GetAll(DBConnectionManager* manager, IntensiveDiscussionFile* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, file_url FROM intensive_discussion_file");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(IntensiveDiscussionFile));
