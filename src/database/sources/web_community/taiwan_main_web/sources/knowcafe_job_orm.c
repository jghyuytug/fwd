...
int KnowcafeJob_Add(DBConnectionManager* manager, const KnowcafeJob* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO knowcafe_job (notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, adorn, adorn_color1, adorn_color2, file, depth, sequence) "
...
int KnowcafeJob_Get(DBConnectionManager* manager, int no, KnowcafeJob* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, adorn, adorn_color1, adorn_color2, file, depth, sequence FROM knowcafe_job WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(KnowcafeJob));
...
int KnowcafeJob_GetAll(DBConnectionManager* manager, KnowcafeJob* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, adorn, adorn_color1, adorn_color2, file, depth, sequence FROM knowcafe_job");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(KnowcafeJob));
