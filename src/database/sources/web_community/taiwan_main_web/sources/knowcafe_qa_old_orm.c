...
int KnowcafeQaOld_Get(DBConnectionManager* manager, int no, KnowcafeQaOld* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, adorn, adorn_color1, adorn_color2, file, depth, sequence, reply FROM knowcafe_qa_old WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(KnowcafeQaOld));
    ...
    record->adorn = row[16] ? atoi(row[16]) : 0;
    record->adorn_color1 = row[17] ? atoi(row[17]) : 0;
    record->adorn_color2 = row[18] ? atoi(row[18]) : 0;
    record->file = row[19] ? atoi(row[19]) : 0;
    record->depth = row[20] ? atoi(row[20]) : 0;
    record->sequence = row[21] ? atof(row[21]) : 0;
    record->reply = row[22] ? atoi(row[22]) : 0;
...
int KnowcafeQaOld_GetAll(DBConnectionManager* manager, KnowcafeQaOld* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, adorn, adorn_color1, adorn_color2, file, depth, sequence, reply FROM knowcafe_qa_old");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(KnowcafeQaOld));
        ...
        records[count].adorn = row[16] ? atoi(row[16]) : 0;
        records[count].adorn_color1 = row[17] ? atoi(row[17]) : 0;
        records[count].adorn_color2 = row[18] ? atoi(row[18]) : 0;
        records[count].file = row[19] ? atoi(row[19]) : 0;
        records[count].depth = row[20] ? atoi(row[20]) : 0;
        records[count].sequence = row[21] ? atof(row[21]) : 0;
        records[count].reply = row[22] ? atoi(row[22]) : 0;
        count++;
    }
