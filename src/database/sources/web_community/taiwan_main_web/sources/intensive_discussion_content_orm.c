#include "intensive_discussion_content_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int IntensiveDiscussionContent_Add(DBConnectionManager* manager, const IntensiveDiscussionContent* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO intensive_discussion_content (no, content, content_type, ip, ring, sms) "
        "VALUES (%d, '%s', %d, '%s', %d, %d)",
...
int IntensiveDiscussionContent_Get(DBConnectionManager* manager, int no, IntensiveDiscussionContent* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM intensive_discussion_content WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(IntensiveDiscussionContent));
...
int IntensiveDiscussionContent_GetAll(DBConnectionManager* manager, IntensiveDiscussionContent* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM intensive_discussion_content");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(IntensiveDiscussionContent));
