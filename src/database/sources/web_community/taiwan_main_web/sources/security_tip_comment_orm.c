#include "security_tip_comment_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int SecurityTipComment_Add(DBConnectionManager* manager, const SecurityTipComment* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO security_tip_comment (mz_id, m_id, m_sex, server_id, charac_no, charac_name, job, comment_text, create_day) "
...
int SecurityTipComment_Get(DBConnectionManager* manager, int no, SecurityTipComment* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, mz_id, m_id, m_sex, server_id, charac_no, charac_name, job, comment_text, create_day FROM security_tip_comment WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(SecurityTipComment));
...
int SecurityTipComment_GetAll(DBConnectionManager* manager, SecurityTipComment* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, mz_id, m_id, m_sex, server_id, charac_no, charac_name, job, comment_text, create_day FROM security_tip_comment");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(SecurityTipComment));
