...
int Penpic2Search_Add(DBConnectionManager* manager, const Penpic2Search* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO penpic2_search (no, penpic_type, category, class, word, degree) "
        "VALUES (%d, %d, %d, %d, '%s', %d)",
...
int Penpic2Search_GetAll(DBConnectionManager* manager, Penpic2Search* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, penpic_type, category, class, word, degree FROM penpic2_search");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(Penpic2Search));
