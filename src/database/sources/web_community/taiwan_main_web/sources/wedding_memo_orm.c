...
int WeddingMemo_Add(DBConnectionManager* manager, const WeddingMemo* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO wedding_memo (guild_id, m_id, charac_no, charac_name, nick_name, memo, create_time) "
        "VALUES (%d, %d, %d, '%s', '%s', '%s', '%s')",
        record->guild_id, record->m_id, record->charac_no, record->charac_name, record->nick_name, record->memo, record->create_time);
...
int WeddingMemo_Get(DBConnectionManager* manager, int no, WeddingMemo* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, guild_id, m_id, charac_no, charac_name, nick_name, memo, create_time FROM wedding_memo WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(WeddingMemo));
...
int WeddingMemo_GetAll(DBConnectionManager* manager, WeddingMemo* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, guild_id, m_id, charac_no, charac_name, nick_name, memo, create_time FROM wedding_memo");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(WeddingMemo));
