...
int TcgRuleFile_Add(DBConnectionManager* manager, const TcgRuleFile* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO tcg_rule_file (content_no, file_url) "
        "VALUES (%d, '%s')",
...
int TcgRuleFile_Get(DBConnectionManager* manager, int no, TcgRuleFile* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, file_url FROM tcg_rule_file WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(TcgRuleFile));
...
int TcgRuleFile_GetAll(DBConnectionManager* manager, TcgRuleFile* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, file_url FROM tcg_rule_file");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(TcgRuleFile));
