...
int Magazine_Add(DBConnectionManager* manager, const Magazine* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO magazine (degree, category, seq, title, reg_date, hists, m_id, user_id, mod_date, search_word, thumbnail_url, writer, summary, open_flag) "
...
int Magazine_Get(DBConnectionManager* manager, int mz_id, Magazine* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT mz_id, degree, category, seq, title, reg_date, hists, m_id, user_id, mod_date, search_word, thumbnail_url, writer, summary, open_flag FROM magazine WHERE mz_id = %d",
        mz_id);
    ...
    memset(record, 0, sizeof(Magazine));
...
int Magazine_GetAll(DBConnectionManager* manager, Magazine* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT mz_id, degree, category, seq, title, reg_date, hists, m_id, user_id, mod_date, search_word, thumbnail_url, writer, summary, open_flag FROM magazine");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(Magazine));
