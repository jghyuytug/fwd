#include "knowcafe_qa_file_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int KnowcafeQaFile_Add(DBConnectionManager* manager, const KnowcafeQaFile* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO knowcafe_qa_file (content_no, file_url) "
        "VALUES (%d, '%s')",
...
int KnowcafeQaFile_Get(DBConnectionManager* manager, int no, KnowcafeQaFile* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, file_url FROM knowcafe_qa_file WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(KnowcafeQaFile));
...
int KnowcafeQaFile_GetAll(DBConnectionManager* manager, KnowcafeQaFile* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, file_url FROM knowcafe_qa_file");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(KnowcafeQaFile));
