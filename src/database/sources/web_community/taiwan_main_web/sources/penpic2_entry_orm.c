...
int Penpic2Entry_Add(DBConnectionManager* manager, const Penpic2Entry* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO penpic2_entry (m_id, penpic_type, cnt) "
        "VALUES (%d, %d, %d)",
...
int Penpic2Entry_Get(DBConnectionManager* manager, int m_id, signed char penpic_type, Penpic2Entry* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT m_id, penpic_type, cnt FROM penpic2_entry WHERE m_id = %d AND penpic_type = %d",
        m_id, penpic_type);
    ...
    memset(record, 0, sizeof(Penpic2Entry));
...
int Penpic2Entry_GetAll(DBConnectionManager* manager, Penpic2Entry* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT m_id, penpic_type, cnt FROM penpic2_entry");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(Penpic2Entry));
