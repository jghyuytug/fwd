...
int Penpic2Comment_Add(DBConnectionManager* manager, const Penpic2Comment* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO penpic2_comment (content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, point, ip, create_day) "
        "VALUES (%d, '%s', %d, '%s', %d, %d, %d, '%s', %d, '%s', %d, '%s', %d)",
        record->content_no, record->m_nickname, record->m_id, record->m_user_id, record->m_sex, record->server_id, record->charac_no, record->charac_name, record->job, record->comment_text, record->point, record->ip, record->create_day);
...
int Penpic2Comment_Get(DBConnectionManager* manager, int no, Penpic2Comment* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, point, ip, create_day FROM penpic2_comment WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(Penpic2Comment));
    ...
    record->point = row[11] ? atoi(row[11]) : 0;
...
int Penpic2Comment_GetAll(DBConnectionManager* manager, Penpic2Comment* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, point, ip, create_day FROM penpic2_comment");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(Penpic2Comment));
        ...
        records[count].point = row[11] ? atoi(row[11]) : 0;
