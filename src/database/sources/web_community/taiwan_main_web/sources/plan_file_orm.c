...
int PlanFile_Add(DBConnectionManager* manager, const PlanFile* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO plan_file (content_no, file_url) "
        "VALUES (%d, '%s')",
...
int PlanFile_Get(DBConnectionManager* manager, int no, PlanFile* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, file_url FROM plan_file WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(PlanFile));
...
int PlanFile_GetAll(DBConnectionManager* manager, PlanFile* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, file_url FROM plan_file");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(PlanFile));
