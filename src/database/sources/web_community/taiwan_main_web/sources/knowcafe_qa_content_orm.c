#include "knowcafe_qa_content_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int KnowcafeQaContent_Add(DBConnectionManager* manager, const KnowcafeQaContent* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO knowcafe_qa_content (no, content, content_type, ip, ring, sms) "
        "VALUES (%d, '%s', %d, '%s', %d, %d)",
...
int KnowcafeQaContent_Get(DBConnectionManager* manager, int no, KnowcafeQaContent* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM knowcafe_qa_content WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(KnowcafeQaContent));
...
int KnowcafeQaContent_GetAll(DBConnectionManager* manager, KnowcafeQaContent* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM knowcafe_qa_content");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(KnowcafeQaContent));
