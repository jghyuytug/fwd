#include "knowledge_content_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int KnowledgeContent_Add(DBConnectionManager* manager, const KnowledgeContent* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO knowledge_content (no, content, content_type, ip) "
        "VALUES (%d, '%s', %d, '%s')",
...
int KnowledgeContent_Get(DBConnectionManager* manager, int no, KnowledgeContent* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip FROM knowledge_content WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(KnowledgeContent));
...
int KnowledgeContent_GetAll(DBConnectionManager* manager, KnowledgeContent* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip FROM knowledge_content");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(KnowledgeContent));
