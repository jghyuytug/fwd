#include "dnf_security_tip_quiz_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int DnfSecurityTipQuiz_Add(DBConnectionManager* manager, const DnfSecurityTipQuiz* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO dnf_security_tip_quiz (magazine_id, m_id, occ_time) "
        "VALUES (%d, %d, '%s')",
...
int DnfSecurityTipQuiz_Get(DBConnectionManager* manager, int magazine_id, int m_id, DnfSecurityTipQuiz* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT magazine_id, m_id, occ_time FROM dnf_security_tip_quiz WHERE magazine_id = %d AND m_id = %d",
        magazine_id, m_id);
    ...
    memset(record, 0, sizeof(DnfSecurityTipQuiz));
...
int DnfSecurityTipQuiz_GetAll(DBConnectionManager* manager, DnfSecurityTipQuiz* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT magazine_id, m_id, occ_time FROM dnf_security_tip_quiz");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(DnfSecurityTipQuiz));
