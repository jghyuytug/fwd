...
int Penpic4_Add(DBConnectionManager* manager, const Penpic4* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO penpic4 (penpic_type, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, point, adorn, adorn_color1, adorn_color2, degree) "
...
int Penpic4_Get(DBConnectionManager* manager, int no, Penpic4* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, penpic_type, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, point, adorn, adorn_color1, adorn_color2, degree FROM penpic4 WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(Penpic4));
...
int Penpic4_GetAll(DBConnectionManager* manager, Penpic4* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, penpic_type, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, point, adorn, adorn_color1, adorn_color2, degree FROM penpic4");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(Penpic4));
