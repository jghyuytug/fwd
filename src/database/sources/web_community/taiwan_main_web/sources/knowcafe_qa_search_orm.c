#include "knowcafe_qa_search_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int KnowcafeQaSearch_Add(DBConnectionManager* manager, const KnowcafeQaSearch* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO knowcafe_qa_search (no, category, class, word, depth) "
        "VALUES (%d, %d, %d, '%s', %d)",
...
int KnowcafeQaSearch_GetAll(DBConnectionManager* manager, KnowcafeQaSearch* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, category, class, word, depth FROM knowcafe_qa_search");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(KnowcafeQaSearch));
