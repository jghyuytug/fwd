#include "knowstandby_file_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int KnowstandbyFile_Add(DBConnectionManager* manager, const KnowstandbyFile* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO knowstandby_file (content_no, file_url) "
        "VALUES (%d, '%s')",
...
int KnowstandbyFile_Get(DBConnectionManager* manager, int no, KnowstandbyFile* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, file_url FROM knowstandby_file WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(KnowstandbyFile));
...
int KnowstandbyFile_GetAll(DBConnectionManager* manager, KnowstandbyFile* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, file_url FROM knowstandby_file");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(KnowstandbyFile));
