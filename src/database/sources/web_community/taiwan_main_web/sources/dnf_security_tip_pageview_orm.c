#include "dnf_security_tip_pageview_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int DnfSecurityTipPageview_Add(DBConnectionManager* manager, const DnfSecurityTipPageview* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO dnf_security_tip_pageview (magazine_id, pageview) "
        "VALUES (%d, %d)",
...
int DnfSecurityTipPageview_Get(DBConnectionManager* manager, int magazine_id, DnfSecurityTipPageview* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT magazine_id, pageview FROM dnf_security_tip_pageview WHERE magazine_id = %d",
        magazine_id);
    ...
    memset(record, 0, sizeof(DnfSecurityTipPageview));
...
int DnfSecurityTipPageview_GetAll(DBConnectionManager* manager, DnfSecurityTipPageview* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT magazine_id, pageview FROM dnf_security_tip_pageview");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(DnfSecurityTipPageview));
