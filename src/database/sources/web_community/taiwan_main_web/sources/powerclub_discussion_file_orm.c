#include "powerclub_discussion_file_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int PowerclubDiscussionFile_Add(DBConnectionManager* manager, const PowerclubDiscussionFile* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO powerclub_discussion_file (content_no, file_url) "
        "VALUES (%d, '%s')",
...
int PowerclubDiscussionFile_Get(DBConnectionManager* manager, int no, PowerclubDiscussionFile* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, file_url FROM powerclub_discussion_file WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(PowerclubDiscussionFile));
...
int PowerclubDiscussionFile_GetAll(DBConnectionManager* manager, PowerclubDiscussionFile* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, file_url FROM powerclub_discussion_file");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(PowerclubDiscussionFile));
