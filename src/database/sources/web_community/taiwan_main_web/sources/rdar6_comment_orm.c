...
int Rdar6Comment_Add(DBConnectionManager* manager, const Rdar6Comment* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO rdar6_comment (content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day) "
...
int Rdar6Comment_Get(DBConnectionManager* manager, int no, Rdar6Comment* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day FROM rdar6_comment WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(Rdar6Comment));
...
int Rdar6Comment_GetAll(DBConnectionManager* manager, Rdar6Comment* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day FROM rdar6_comment");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(Rdar6Comment));
