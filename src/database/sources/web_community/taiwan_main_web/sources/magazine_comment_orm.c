#include "magazine_comment_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int MagazineComment_Add(DBConnectionManager* manager, const MagazineComment* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO magazine_comment (mz_id, m_id, m_sex, server_id, charac_no, charac_name, job, comment_text, create_day) "
...
int MagazineComment_Get(DBConnectionManager* manager, int no, MagazineComment* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, mz_id, m_id, m_sex, server_id, charac_no, charac_name, job, comment_text, create_day FROM magazine_comment WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(MagazineComment));
...
int MagazineComment_GetAll(DBConnectionManager* manager, MagazineComment* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, mz_id, m_id, m_sex, server_id, charac_no, charac_name, job, comment_text, create_day FROM magazine_comment");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(MagazineComment));
