...
int KnowledgeTest_Add(DBConnectionManager* manager, const KnowledgeTest* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO knowledge_test (kn_id, question_flag, m_id, server_id, charac_no, charac_name, job, title, know_flag, create_day, comment, view, value, file) "
...
int KnowledgeTest_Get(DBConnectionManager* manager, int no, KnowledgeTest* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, kn_id, question_flag, m_id, server_id, charac_no, charac_name, job, title, know_flag, create_day, comment, view, value, file FROM knowledge_test WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(KnowledgeTest));
...
int KnowledgeTest_GetAll(DBConnectionManager* manager, KnowledgeTest* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, kn_id, question_flag, m_id, server_id, charac_no, charac_name, job, title, know_flag, create_day, comment, view, value, file FROM knowledge_test");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(KnowledgeTest));
