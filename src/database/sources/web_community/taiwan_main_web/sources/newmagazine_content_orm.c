#include "newmagazine_content_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int NewmagazineContent_Add(DBConnectionManager* manager, const NewmagazineContent* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO newmagazine_content (mz_id, content) "
        "VALUES (%d, '%s')",
...
int NewmagazineContent_Get(DBConnectionManager* manager, int mz_id, NewmagazineContent* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT mz_id, content FROM newmagazine_content WHERE mz_id = %d",
        mz_id);
    ...
    memset(record, 0, sizeof(NewmagazineContent));
...
int NewmagazineContent_GetAll(DBConnectionManager* manager, NewmagazineContent* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT mz_id, content FROM newmagazine_content");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(NewmagazineContent));
