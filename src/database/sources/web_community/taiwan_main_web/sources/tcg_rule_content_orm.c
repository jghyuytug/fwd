#include "tcg_rule_content_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int TcgRuleContent_Add(DBConnectionManager* manager, const TcgRuleContent* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO tcg_rule_content (no, content, content_type, ip, ring, sms) "
        "VALUES (%d, '%s', %d, '%s', %d, %d)",
...
int TcgRuleContent_Get(DBConnectionManager* manager, int no, TcgRuleContent* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM tcg_rule_content WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(TcgRuleContent));
...
int TcgRuleContent_GetAll(DBConnectionManager* manager, TcgRuleContent* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM tcg_rule_content");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(TcgRuleContent));
