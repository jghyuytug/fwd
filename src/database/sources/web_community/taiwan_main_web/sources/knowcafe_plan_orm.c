...
int KnowcafePlan_Add(DBConnectionManager* manager, const KnowcafePlan* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO knowcafe_plan (notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, sub_job, lev, title, create_day, comment, view, recom, file) "
...
int KnowcafePlan_Get(DBConnectionManager* manager, int no, KnowcafePlan* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, sub_job, lev, title, create_day, comment, view, recom, file FROM knowcafe_plan WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(KnowcafePlan));
...
int KnowcafePlan_GetAll(DBConnectionManager* manager, KnowcafePlan* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, sub_job, lev, title, create_day, comment, view, recom, file FROM knowcafe_plan");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(KnowcafePlan));
