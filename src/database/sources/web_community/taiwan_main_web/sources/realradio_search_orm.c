#include "realradio_search_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int RealradioSearch_Add(DBConnectionManager* manager, const RealradioSearch* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO realradio_search (no, bbs_type, category, class, word) "
        "VALUES (%d, %d, %d, %d, '%s')",
...
int RealradioSearch_GetAll(DBConnectionManager* manager, RealradioSearch* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, bbs_type, category, class, word FROM realradio_search");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(RealradioSearch));
