#include "member_passwd_mod_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#define MAX_QUERY_LEN 8192

int MemberPasswdMod_Add(DBConnectionManager* manager, const MemberPasswdMod* record) {
    char query[MAX_QUERY_LEN];
    DBQueryResult result;

    if (!manager || !record) return -1;

    snprintf(query, sizeof(query),
        "INSERT INTO member_passwd_mod (m_id, first_time, last_time, cnt) "
        "VALUES (%d, '%s', '%s', %d)",
        record->m_id, record->first_time, record->last_time, record->cnt);

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_TW, query, &result) < 0) {
        return -1;
    }

    DBQueryResult_Free(&result);
    return 0;
}

int MemberPasswdMod_Get(DBConnectionManager* manager, int m_id, MemberPasswdMod* record) {
    char query[MAX_QUERY_LEN];
    DBQueryResult result;
    char* row[4];

    if (!manager || !record) return -1;

    snprintf(query, sizeof(query),
        "SELECT m_id, first_time, last_time, cnt FROM member_passwd_mod WHERE m_id = %d",
        m_id);

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_TW, query, &result) < 0)
        return -1;

    if (DBQueryResult_FetchRow(&result, row) <= 0) {
        DBQueryResult_Free(&result);
        return -1;
    }

    memset(record, 0, sizeof(MemberPasswdMod));
    record->m_id = row[0] ? atoi(row[0]) : 0;
    if (row[1]) strncpy(record->first_time, row[1], sizeof(record->first_time) - 1);
    if (row[2]) strncpy(record->last_time, row[2], sizeof(record->last_time) - 1);
    record->cnt = row[3] ? atoi(row[3]) : 0;

    DBQueryResult_Free(&result);
    return 0;
}

int MemberPasswdMod_GetAll(DBConnectionManager* manager, MemberPasswdMod* records, int max_count, int* actual_count) {
    char query[MAX_QUERY_LEN];
    DBQueryResult result;
    char* row[4];
    int count = 0;

    if (!manager || !records || !actual_count) return -1;

    snprintf(query, sizeof(query),
        "SELECT m_id, first_time, last_time, cnt FROM member_passwd_mod");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_TW, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(MemberPasswdMod));
        records[count].m_id = row[0] ? atoi(row[0]) : 0;
        if (row[1]) strncpy(records[count].first_time, row[1], sizeof(records[count].first_time) - 1);
        if (row[2]) strncpy(records[count].last_time, row[2], sizeof(records[count].last_time) - 1);
        records[count].cnt = row[3] ? atoi(row[3]) : 0;
        count++;
    }

    *actual_count = count;
    DBQueryResult_Free(&result);
    return 0;
}
