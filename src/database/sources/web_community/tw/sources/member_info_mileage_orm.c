...
int MemberInfoMileage_Add(DBConnectionManager* manager, const MemberInfoMileage* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO member_info_mileage (user_id, user_name, first_ssn, second_ssn, passwd, mobile_no, reg_date, email, q_no, q_answer, updt_date, state, nickname, email_yn, ssn_check, slot, last_play_time, hangame_flag, hanmon_flag, mileage) "
        "VALUES ('%s', '%s', '%s', '%s', '%s', '%s', %d, '%s', %d, '%s', '%s', %d, '%s', %d, %d, %d, '%s', %d, %d, %d)",
        record->user_id, record->user_name, record->first_ssn, record->second_ssn, record->passwd, record->mobile_no, record->reg_date, record->email, record->q_no, record->q_answer, record->updt_date, record->state, record->nickname, record->email_yn, record->ssn_check, record->slot, record->last_play_time, record->hangame_flag, record->hanmon_flag, record->mileage);
...
int MemberInfoMileage_Get(DBConnectionManager* manager, int m_id, MemberInfoMileage* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT m_id, user_id, user_name, first_ssn, second_ssn, passwd, mobile_no, reg_date, email, q_no, q_answer, updt_date, state, nickname, email_yn, ssn_check, slot, last_play_time, hangame_flag, hanmon_flag, mileage FROM member_info_mileage WHERE m_id = %d",
        m_id);
    ...
    memset(record, 0, sizeof(MemberInfoMileage));
    ...
    record->slot = row[16] ? atoi(row[16]) : 0;
    if (row[17]) strncpy(record->last_play_time, row[17], sizeof(record->last_play_time) - 1);
    record->hangame_flag = row[18] ? atoi(row[18]) : 0;
    record->hanmon_flag = row[19] ? atoi(row[19]) : 0;
    record->mileage = row[20] ? atoi(row[20]) : 0;
...
int MemberInfoMileage_GetAll(DBConnectionManager* manager, MemberInfoMileage* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT m_id, user_id, user_name, first_ssn, second_ssn, passwd, mobile_no, reg_date, email, q_no, q_answer, updt_date, state, nickname, email_yn, ssn_check, slot, last_play_time, hangame_flag, hanmon_flag, mileage FROM member_info_mileage");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_TW, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(MemberInfoMileage));
        ...
        records[count].slot = row[16] ? atoi(row[16]) : 0;
        if (row[17]) strncpy(records[count].last_play_time, row[17], sizeof(records[count].last_play_time) - 1);
        records[count].hangame_flag = row[18] ? atoi(row[18]) : 0;
        records[count].hanmon_flag = row[19] ? atoi(row[19]) : 0;
        records[count].mileage = row[20] ? atoi(row[20]) : 0;
        count++;
    }
