...
int SlangList_Add(DBConnectionManager* manager, const char* slang) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO slang_list (slang) VALUES ('%s')", escaped_slang);
...
int SlangList_Exists(DBConnectionManager* manager, const char* slang) {
    ...
    snprintf(query, sizeof(query),
        "SELECT COUNT(*) FROM slang_list WHERE slang = '%s'", escaped_slang);
...
int SlangList_Delete(DBConnectionManager* manager, const char* slang) {
    ...
    snprintf(query, sizeof(query),
        "DELETE FROM slang_list WHERE slang = '%s'", escaped_slang);
...
int SlangList_GetAll(DBConnectionManager* manager, SlangList* slang_list,
    ...
    snprintf(query, sizeof(query),
        "SELECT slang FROM slang_list ORDER BY slang LIMIT %d", max_count);
    ...
    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&slang_list[count], 0, sizeof(SlangList));
        if (row[0]) {
...
int SlangList_Count(DBConnectionManager* manager, int* count) {
    ...
    snprintf(query, sizeof(query), "SELECT COUNT(*) FROM slang_list");
...
int SlangList_Clear(DBConnectionManager* manager) {
    ...
    snprintf(query, sizeof(query), "DELETE FROM slang_list");
...
int SlangList_IsFiltered(DBConnectionManager* manager, const char* text) {
    ...
    /* Get all slang words */
    snprintf(query, sizeof(query), "SELECT slang FROM slang_list");
