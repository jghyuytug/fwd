...
int AccountCargo_GetByID(DBConnectionManager* manager, int m_id, AccountCargo* cargo) {
    ...
    printf("[DEBUG] hex_str pointer: %p\n", (void*)hex_str);
    if (hex_str) {
        printf("[DEBUG] hex_str length: %lu\n", strlen(hex_str));
        printf("[DEBUG] hex_str first 40 chars: %.40s\n", hex_str);
    } else {
        printf("[DEBUG] hex_str is NULL\n");
    }

    if (hex_str && strlen(hex_str) > 0) {
        cargo->cargo_data = DecodeHexString(hex_str, &cargo->cargo_data_len);
        printf("[DEBUG] Decoded cargo_data_len: %d\n", cargo->cargo_data_len);
    } else {
        cargo->cargo_data = NULL;
        cargo->cargo_data_len = 0;
        printf("[DEBUG] hex_str is NULL or empty, setting cargo_data_len=0\n");
    }
