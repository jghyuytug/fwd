#include "event_account_fatigue_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int EventAccountFatigue_Add(DBConnectionManager* manager, const EventAccountFatigue* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO event_account_fatigue (m_id, fatigue) "
        "VALUES (%d, %d)",
        record->m_id, record->fatigue);
...
int EventAccountFatigue_Get(DBConnectionManager* manager, int m_id, EventAccountFatigue* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT m_id, fatigue FROM event_account_fatigue WHERE m_id = %d",
        m_id);
    ...
    memset(record, 0, sizeof(EventAccountFatigue));
...
int EventAccountFatigue_GetAll(DBConnectionManager* manager, EventAccountFatigue* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT m_id, fatigue FROM event_account_fatigue");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_GAME_EVENT, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(EventAccountFatigue));
