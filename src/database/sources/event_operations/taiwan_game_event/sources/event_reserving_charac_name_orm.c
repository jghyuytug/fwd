#include "event_reserving_charac_name_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int EventReservingCharacName_Add(DBConnectionManager* manager, const EventReservingCharacName* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO event_reserving_charac_name (user_id, server_info, charac_name) "
        "VALUES ('%s', %d, '%s')",
...
int EventReservingCharacName_Get(DBConnectionManager* manager, signed char server_info, const char* charac_name, EventReservingCharacName* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT user_id, server_info, charac_name FROM event_reserving_charac_name WHERE server_info = %d AND charac_name = '%s'",
    ...
    memset(record, 0, sizeof(EventReservingCharacName));
...
int EventReservingCharacName_GetAll(DBConnectionManager* manager, EventReservingCharacName* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT user_id, server_info, charac_name FROM event_reserving_charac_name");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_GAME_EVENT, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(EventReservingCharacName));
