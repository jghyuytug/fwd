#include "event_valentine_vote_history_old_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int EventValentineVoteHistoryOld_Add(DBConnectionManager* manager, const EventValentineVoteHistoryOld* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO event_valentine_vote_history_old (check_date, server_id, vote_A, vote_B, vote_C) "
        "VALUES ('%s', %d, %d, %d, %d)",
...
int EventValentineVoteHistoryOld_Get(DBConnectionManager* manager, const char* check_date, signed char server_id, EventValentineVoteHistoryOld* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT check_date, server_id, vote_A, vote_B, vote_C FROM event_valentine_vote_history_old WHERE check_date = '%s' AND server_id = %d",
    ...
    memset(record, 0, sizeof(EventValentineVoteHistoryOld));
...
int EventValentineVoteHistoryOld_GetAll(DBConnectionManager* manager, EventValentineVoteHistoryOld* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT check_date, server_id, vote_A, vote_B, vote_C FROM event_valentine_vote_history_old");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_GAME_EVENT, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(EventValentineVoteHistoryOld));
