...
int UserItems_Add(DBConnectionManager* manager, const UserItems* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO user_items (charac_no, slot, it_id, expire_date, obtain_from, reg_date, ipg_agency_no, ability_no, stat, clear_avatar_id, item_lock_key, to_ipg_agency_no, m_time, hidden_option, emblem_endurance, color1, color2, trade_restrict) "
...
int UserItems_Get(DBConnectionManager* manager, int ui_id, UserItems* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT ui_id, charac_no, slot, it_id, expire_date, obtain_from, reg_date, ipg_agency_no, ability_no, stat, clear_avatar_id, item_lock_key, to_ipg_agency_no, m_time, hidden_option, emblem_endurance, color1, color2, trade_restrict FROM user_items WHERE ui_id = %d",
        ui_id);
    ...
    memset(record, 0, sizeof(UserItems));
...
int UserItems_GetAll(DBConnectionManager* manager, UserItems* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT ui_id, charac_no, slot, it_id, expire_date, obtain_from, reg_date, ipg_agency_no, ability_no, stat, clear_avatar_id, item_lock_key, to_ipg_agency_no, m_time, hidden_option, emblem_endurance, color1, color2, trade_restrict FROM user_items");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_CAIN, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(UserItems));
