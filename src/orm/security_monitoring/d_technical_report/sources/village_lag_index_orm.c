#include "village_lag_index_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int VillageLagIndex_Add(DBConnectionManager* manager, const VillageLagIndex* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO village_lag_index (spec_id, occ_time, server_group, min_fps, avg_fps, max_fps, win_fps, full_fps, full_win_fps, full_win_nosync_fps, frame1, time1, frame2, time2, frame3, time3, frame4, time4, frame5, time5, frame6, time6, share_rate) "
...
int VillageLagIndex_GetAll(DBConnectionManager* manager, VillageLagIndex* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT spec_id, occ_time, server_group, min_fps, avg_fps, max_fps, win_fps, full_fps, full_win_fps, full_win_nosync_fps, frame1, time1, frame2, time2, frame3, time3, frame4, time4, frame5, time5, frame6, time6, share_rate FROM village_lag_index");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_TECH_REPORT, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(VillageLagIndex));
