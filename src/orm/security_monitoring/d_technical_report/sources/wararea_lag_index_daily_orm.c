#include "wararea_lag_index_daily_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int WarareaLagIndexDaily_Add(DBConnectionManager* manager, const WarareaLagIndexDaily* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO wararea_lag_index_daily (occ_date, spec_id, win_fps, win_fps_cnt, full_fps, full_fps_cnt, full_win_fps, full_win_fps_cnt, full_win_nosync_fps, full_win_nosync_fps_cnt) "
...
int WarareaLagIndexDaily_Get(DBConnectionManager* manager, const char* occ_date, int spec_id, WarareaLagIndexDaily* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT occ_date, spec_id, win_fps, win_fps_cnt, full_fps, full_fps_cnt, full_win_fps, full_win_fps_cnt, full_win_nosync_fps, full_win_nosync_fps_cnt FROM wararea_lag_index_daily WHERE occ_date = '%s' AND spec_id = %d",
        occ_date, spec_id);
    ...
    memset(record, 0, sizeof(WarareaLagIndexDaily));
...
int WarareaLagIndexDaily_GetAll(DBConnectionManager* manager, WarareaLagIndexDaily* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT occ_date, spec_id, win_fps, win_fps_cnt, full_fps, full_fps_cnt, full_win_fps, full_win_fps_cnt, full_win_nosync_fps, full_win_nosync_fps_cnt FROM wararea_lag_index_daily");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_TECH_REPORT, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(WarareaLagIndexDaily));
