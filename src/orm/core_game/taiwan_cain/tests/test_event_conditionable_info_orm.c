#include "event_conditionable_info_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <assert.h>
...
void test_add_and_get(DBConnectionManager* manager) {
    ...
    ASSERT_EQ(EventConditionableInfo_Add(manager, &record), 0, "添加记录成功");

    EventConditionableInfo retrieved;
    memset(&retrieved, 0, sizeof(EventConditionableInfo));
    ASSERT_EQ(EventConditionableInfo_Get(manager, 10001, &retrieved), 0, "获取记录成功");
...
void test_update(DBConnectionManager* manager) {
    ...
    EventConditionableInfo record;
    memset(&record, 0, sizeof(EventConditionableInfo));
    ASSERT_EQ(EventConditionableInfo_Get(manager, 10001, &record), 0, "获取已存在记录");

    record.current_step = 10;
    record.reward_step = 5;
    strcpy(record.update_time, "2024-01-01 11:00:00");

    ASSERT_EQ(EventConditionableInfo_Update(manager, &record), 0, "更新记录成功");

    EventConditionableInfo retrieved;
    memset(&retrieved, 0, sizeof(EventConditionableInfo));
    ASSERT_EQ(EventConditionableInfo_Get(manager, 10001, &retrieved), 0, "获取更新后记录");
...
void test_exists(DBConnectionManager* manager) {
    ...
    ASSERT_TRUE(EventConditionableInfo_Exists(manager, 10001), "记录存在");
    ASSERT_TRUE(!EventConditionableInfo_Exists(manager, 99999), "记录不存在");
}

void test_add_multiple_records(DBConnectionManager* manager) {
    TEST_START("添加多条活动额外条件信息");

    EventConditionableInfo record2;
    memset(&record2, 0, sizeof(EventConditionableInfo));
    ...
    ASSERT_EQ(EventConditionableInfo_Add(manager, &record2), 0, "添加记录2成功");

    EventConditionableInfo record3;
    memset(&record3, 0, sizeof(EventConditionableInfo));
    ...
    ASSERT_EQ(EventConditionableInfo_Add(manager, &record3), 0, "添加记录3成功");

    EventConditionableInfo record4;
    memset(&record4, 0, sizeof(EventConditionableInfo));
    record4.charac_no = 10004;
    record4.current_step = 8;
    record4.reward_step = 5;
    strcpy(record4.update_time, "2024-01-01 14:00:00");
    ASSERT_EQ(EventConditionableInfo_Add(manager, &record4), 0, "添加记录4成功");
}

void test_increment_current_step(DBConnectionManager* manager) {
    TEST_START("增加当前步骤");

    ASSERT_EQ(EventConditionableInfo_IncrementCurrentStep(manager, 10001, 5), 0, "增加当前步骤成功");
...
void test_update_current_step(DBConnectionManager* manager) {
    ...
    ASSERT_EQ(EventConditionableInfo_UpdateCurrentStep(manager, 10001, 25), 0, "更新当前步骤成功");
...
void test_update_reward_step(DBConnectionManager* manager) {
    ...
    ASSERT_EQ(EventConditionableInfo_UpdateRewardStep(manager, 10001, 15), 0, "更新奖励步骤成功");
...
void test_claim_reward(DBConnectionManager* manager) {
    ...
    ASSERT_EQ(EventConditionableInfo_ClaimReward(manager, 10001), 0, "领取奖励成功");
...
void test_reset_progress(DBConnectionManager* manager) {
    ...
    ASSERT_EQ(EventConditionableInfo_ResetProgress(manager, 10002), 0, "重置进度成功");

    EventConditionableInfo retrieved;
    memset(&retrieved, 0, sizeof(EventConditionableInfo));
    ASSERT_EQ(EventConditionableInfo_Get(manager, 10002, &retrieved), 0, "获取记录成功");
...
void test_batch_reset_progress(DBConnectionManager* manager) {
    ...
    ASSERT_EQ(EventConditionableInfo_BatchResetProgress(manager), 0, "批量重置成功");
    ...
    memset(&retrieved, 0, sizeof(EventConditionableInfo));
    ASSERT_EQ(EventConditionableInfo_Get(manager, 10003, &retrieved), 0, "获取记录成功");
...
void test_get_by_current_step(DBConnectionManager* manager) {
    ...
    /* 先设置一些不同的步骤 */
    EventConditionableInfo_UpdateCurrentStep(manager, 10001, 50);
    EventConditionableInfo_UpdateCurrentStep(manager, 10002, 50);
    EventConditionableInfo_UpdateCurrentStep(manager, 10003, 30);

    EventConditionableInfo records[10];
    memset(records, 0, sizeof(records));
    int count = EventConditionableInfo_GetByCurrentStep(manager, 50, records, 10);
...
void test_get_by_min_current_step(DBConnectionManager* manager) {
    ...
    EventConditionableInfo records[10];
    memset(records, 0, sizeof(records));
    int count = EventConditionableInfo_GetByMinCurrentStep(manager, 40, records, 10);
...
void test_get_top_by_current_step(DBConnectionManager* manager) {
    ...
    EventConditionableInfo records[10];
    memset(records, 0, sizeof(records));
    int count = EventConditionableInfo_GetTopByCurrentStep(manager, 3, records, 10);
...
void test_get_unclaimed_rewards(DBConnectionManager* manager) {
    ...
    /* 设置一些有未领取奖励的记录 */
    EventConditionableInfo_UpdateCurrentStep(manager, 10001, 20);
    EventConditionableInfo_UpdateRewardStep(manager, 10001, 10);

    EventConditionableInfo records[10];
    memset(records, 0, sizeof(records));
    int count = EventConditionableInfo_GetUnclaimedRewards(manager, records, 10);
...
void test_get_by_updated_after(DBConnectionManager* manager) {
    ...
    memset(records, 0, sizeof(records));
    /* 使用过去的日期，避免NOW()导致的问题 */
    int count = EventConditionableInfo_GetByUpdatedAfter(manager, "2020-01-01 00:00:00",
                                                               records, 10);
...
void test_count_by_current_step(DBConnectionManager* manager) {
    ...
    int count = EventConditionableInfo_CountByCurrentStep(manager, 50);
...
void test_count_unclaimed_rewards(DBConnectionManager* manager) {
    ...
    int count = EventConditionableInfo_CountUnclaimedRewards(manager);
...
void test_get_average_progress(DBConnectionManager* manager) {
    ...
    double average = EventConditionableInfo_GetAverageProgress(manager);
...
void test_delete(DBConnectionManager* manager) {
    ...
    ASSERT_EQ(EventConditionableInfo_Delete(manager, 10001), 0, "删除记录成功");
    ASSERT_TRUE(!EventConditionableInfo_Exists(manager, 10001), "记录已删除");

    ASSERT_EQ(EventConditionableInfo_Delete(manager, 10002), 0, "删除记录2成功");
    ASSERT_EQ(EventConditionableInfo_Delete(manager, 10003), 0, "删除记录3成功");
    ASSERT_EQ(EventConditionableInfo_Delete(manager, 10004), 0, "删除记录4成功");
}
...
void test_print_info(DBConnectionManager* manager) {
    ...
    EventConditionableInfo_Add(manager, &record);

    EventConditionableInfo retrieved;
    memset(&retrieved, 0, sizeof(EventConditionableInfo));
    if (EventConditionableInfo_Get(manager, 99999, &retrieved) == 0) {
        EventConditionableInfo_PrintInfo(&retrieved);
        ASSERT_TRUE(1, "打印信息成功");
        EventConditionableInfo_Delete(manager, 99999);
    } else {
        printf("  ⚠ 无记录可打印\n");
    }
}
