...
int main(int argc, char* argv[]) {
    ...
    printf("=== Member Mousepass History 201506 ORM Test Suite ===\n\n");
...
void cleanup_test_data(DBConnectionManager* manager) {
    ...
    snprintf(query, sizeof(query),
        "DELETE FROM member_mousepass_history_201506 WHERE m_id IN (100001, 100002)");
...
void test_basic_crud(DBConnectionManager* manager) {
    ...
    strcpy(record.occ_time, "2015-06-15 10:00:00");
    record.m_id = 100001;
    ...
    if (MemberMousepassHistory201506_Add(manager, &record) == 0) {
    ...
    test_total++;
    exists = MemberMousepassHistory201506_Exists(manager, "2015-06-15 10:00:00", 100001);
    if (exists) {
    ...
    test_total++;
    memset(&record, 0, sizeof(MemberMousepassHistory201506));
    if (MemberMousepassHistory201506_Get(manager, "2015-06-15 10:00:00", 100001, &record) == 0) {
    ...
    test_total++;
    if (MemberMousepassHistory201506_Delete(manager, "2015-06-15 10:00:00", 100001) == 0) {
        exists = MemberMousepassHistory201506_Exists(manager, "2015-06-15 10:00:00", 100001);
        if (!exists) {
...
void test_business_operations(DBConnectionManager* manager) {
    MemberMousepassHistory201506 record;
    MemberMousepassHistory201506 records[10];
    int actual_count = 0;
    ...
    strcpy(record.port_info, "8080");
    record.modify_type = 1;
    MemberMousepassHistory201506_Add(manager, &record);

    strcpy(record.occ_time, "2015-06-15 11:00:00");
    record.m_id = 100001;
    ...
    strcpy(record.occ_time, "2015-06-15 12:00:00");
    record.m_id = 100002;
    ...
    test_total++;
    if (MemberMousepassHistory201506_GetByMemberId(manager, 100001, records, 10, &actual_count) == 0) {
        if (actual_count >= 2) {
    ...
    test_total++;
    if (MemberMousepassHistory201506_GetByIP(manager, "192.168.1.1", records, 10, &actual_count) == 0) {
        if (actual_count >= 2) {
    ...
    test_total++;
    if (MemberMousepassHistory201506_GetByTimeRange(manager,
                                                     "2015-06-15 09:00:00",
                                                     "2015-06-15 11:30:00",
