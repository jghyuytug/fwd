...
void cleanup_test_data(DBConnectionManager* manager) {
    ...
    snprintf(query, sizeof(query),
        "DELETE FROM churn_reward_history_201507 WHERE m_id IN (100001, 100002)");
    DBConnectionManager_ExecuteQuery(manager, DB_TYPE_LOGIN, query, &result);
    DBQueryResult_Free(&result);
}

void test_basic_crud(DBConnectionManager* manager) {
    ChurnRewardHistory201507 history;
    int ret;
    ...
    ret = ChurnRewardHistory201507_Add(manager, &history);
    check_test("Add new record", ret == 0);

    ret = ChurnRewardHistory201507_Exists(manager, 100001, "2025-01-15 10:00:00");
    check_test("Record exists (2 keys)", ret == 1);

    ChurnRewardHistory201507 retrieved;
    ret = ChurnRewardHistory201507_Get(manager, 100001, "2025-01-15 10:00:00", &retrieved);
    ...
    ret = ChurnRewardHistory201507_Delete(manager, 100001, "2025-01-15 10:00:00");
...
void test_business_operations(DBConnectionManager* manager) {
    ChurnRewardHistory201507 history;
    ChurnRewardHistory201507 results[10];
    int actual_count;
    int ret;
    ...
    history.server_id = 1;
    history.cera = 100;
    ChurnRewardHistory201507_Add(manager, &history);

    strcpy(history.occ_time, "2025-01-16 11:00:00");
    history.cera = 200;
    ChurnRewardHistory201507_Add(manager, &history);

    ret = ChurnRewardHistory201507_GetByMember(manager, 100002, results, 10, &actual_count);
    check_test("Get by member", ret == 0 && actual_count == 2);

    ret = ChurnRewardHistory201507_GetByTimeRange(manager, "2025-01-15 00:00:00",
                                                    "2025-01-17 00:00:00",
...
int main(int argc, char* argv[]) {
    ...
    printf("=== Churn Reward History 201507 ORM Test Suite ===\n");
