...
int main(int argc, char* argv[]) {
    ...
    printf("=== Login Account 3 ORM Test Suite ===\n\n");
...
void cleanup_test_data(DBConnectionManager* manager) {
    ...
    snprintf(query, sizeof(query),
        "DELETE FROM login_account_3 WHERE m_id IN (100001, 100002, 100003)");
    DBConnectionManager_ExecuteQuery(manager, DB_TYPE_LOGIN, query, &result);
    DBQueryResult_Free(&result);
}

void test_basic_crud(DBConnectionManager* manager) {
    LoginAccount3 record;
    int exists;
    ...
    if (LoginAccount3_Add(manager, &record) == 0) {
    ...
    test_total++;
    exists = LoginAccount3_Exists(manager, 100001);
    if (exists) {
    ...
    test_total++;
    memset(&record, 0, sizeof(LoginAccount3));
    if (LoginAccount3_Get(manager, 100001, &record) == 0) {
    ...
    if (LoginAccount3_Update(manager, &record) == 0) {
        memset(&record, 0, sizeof(LoginAccount3));
        LoginAccount3_Get(manager, 100001, &record);
    ...
    test_total++;
    if (LoginAccount3_Delete(manager, 100001) == 0) {
        exists = LoginAccount3_Exists(manager, 100001);
        if (!exists) {
...
void test_business_operations(DBConnectionManager* manager) {
    ...
    strcpy(record.last_login_date, "2025-11-19 16:00:00");
    strcpy(record.login_ip, "192.168.1.100");
    LoginAccount3_Add(manager, &record);
    ...
    test_total++;
    if (LoginAccount3_GetByStatus(manager, 1, records, 10, &actual_count) == 0) {
        if (actual_count >= 2) {
    ...
    test_total++;
    if (LoginAccount3_GetByChannel(manager, 1, records, 10, &actual_count) == 0) {
        if (actual_count >= 2) {
    ...
    test_total++;
    if (LoginAccount3_UpdateLoginStatus(manager, 100001, 0) == 0) {
        memset(&record, 0, sizeof(LoginAccount3));
        LoginAccount3_Get(manager, 100001, &record);
    ...
    test_total++;
    if (LoginAccount3_UpdateLastLogin(manager, 100001, "2025-11-19 17:00:00", "192.168.1.200") == 0) {
        memset(&record, 0, sizeof(LoginAccount3));
        LoginAccount3_Get(manager, 100001, &record);
