...
int ChurnRewardHistory201506_Add(DBConnectionManager* manager, const ChurnRewardHistory201506* history) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO churn_reward_history_201506 "
...
int ChurnRewardHistory201506_Get(DBConnectionManager* manager, unsigned int m_id,
    ...
    snprintf(query, sizeof(query),
        ...
        "FROM churn_reward_history_201506 WHERE m_id = %u AND occ_time = '%s'",
        m_id, occ_time);
...
int ChurnRewardHistory201506_Delete(DBConnectionManager* manager, unsigned int m_id, const char* occ_time) {
    ...
    snprintf(query, sizeof(query),
        "DELETE FROM churn_reward_history_201506 WHERE m_id = %u AND occ_time = '%s'",
        m_id, occ_time);
...
int ChurnRewardHistory201506_Exists(DBConnectionManager* manager, unsigned int m_id, const char* occ_time) {
    ...
    snprintf(query, sizeof(query),
        "SELECT COUNT(*) FROM churn_reward_history_201506 WHERE m_id = %u AND occ_time = '%s'",
        m_id, occ_time);
...
int ChurnRewardHistory201506_GetByMember(DBConnectionManager* manager, unsigned int m_id,
    ...
    snprintf(query, sizeof(query),
        "SELECT m_id, occ_time, server_id, charac_no, item_id, add_info, luck_point, reward_order, cera "
        "FROM churn_reward_history_201506 WHERE m_id = %u ORDER BY occ_time DESC", m_id);

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_LOGIN, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&histories[count], 0, sizeof(ChurnRewardHistory201506));
...
int ChurnRewardHistory201506_GetByTimeRange(DBConnectionManager* manager, const char* start_time,
    ...
    snprintf(query, sizeof(query),
        "SELECT m_id, occ_time, server_id, charac_no, item_id, add_info, luck_point, reward_order, cera "
        "FROM churn_reward_history_201506 WHERE occ_time >= '%s' AND occ_time <= '%s' "
...
void ChurnRewardHistory201506_PrintInfo(const ChurnRewardHistory201506* history) {
    ...
    printf("=== Churn Reward History 201506 ===\n");
    printf("Member ID: %u\n", history->m_id);
