...
int main(int argc, char* argv[]) {
    ...
    printf("=== EventGoldcardEntry1 ORM Test Suite ===\n\n");
...
void cleanup_test_data(DBConnectionManager* manager) {
    ...
    snprintf(query, sizeof(query),
        "DELETE FROM event_goldcard_entry1 WHERE m_id IN (100001, 100002, 200001)");
...
void test_basic_crud(DBConnectionManager* manager) {
    EventGoldcardEntry1 entry, retrieved;
    int result;
    ...
    result = EventGoldcardEntry1_Add(manager, &entry);
    if (result == 0) {
        printf("  [PASS] Add entry (occ_date=20251117, m_id=100001, item_no=5)\n");
        test_passed++;
    } else {
    ...
    /* Test 2: Exists check */
    result = EventGoldcardEntry1_Exists(manager, 20251117, 100001);
    if (result == 1) {
    ...
    /* Test 3: Get entry */
    memset(&retrieved, 0, sizeof(EventGoldcardEntry1));
    result = EventGoldcardEntry1_Get(manager, 20251117, 100001, &retrieved);
    if (result == 0 && retrieved.m_id == 100001 && retrieved.item_no == 5) {
    ...
    /* Test 4: Update entry */
    retrieved.item_no = 10;
    result = EventGoldcardEntry1_Update(manager, &retrieved);
    if (result == 0) {
    ...
    memset(&retrieved, 0, sizeof(EventGoldcardEntry1));
    result = EventGoldcardEntry1_Get(manager, 20251117, 100001, &retrieved);
    if (result == 0 && retrieved.item_no == 10) {
        printf("  [PASS] Verify update (item_no=10)\n");
        test_passed++;
    } else {
    ...
    /* Test 6: Delete entry */
    result = EventGoldcardEntry1_Delete(manager, 20251117, 100001);
    if (result == 0) {
...
void test_business_operations(DBConnectionManager* manager) {
    EventGoldcardEntry1 entries[10];
    EventGoldcardEntry1 test_data[3];
    int actual_count;
    int result;
    ...
    test_data[0].item_no = 1;

    test_data[1].occ_date = 20251118;
    test_data[1].m_id = 100001;
    test_data[1].item_no = 2;

    test_data[2].occ_date = 20251117;
    test_data[2].m_id = 200001;
    test_data[2].item_no = 3;

    /* Insert test data */
    for (int i = 0; i < 3; i++) {
        EventGoldcardEntry1_Add(manager, &test_data[i]);
    }

    /* Test 1: GetByMId */
    actual_count = 0;
    memset(entries, 0, sizeof(entries));
    result = EventGoldcardEntry1_GetByMId(manager, 100001, entries, 10, &actual_count);
    ...
    actual_count = 0;
    memset(entries, 0, sizeof(entries));
    result = EventGoldcardEntry1_GetByDate(manager, 20251117, entries, 10, &actual_count);
    if (result == 0 && actual_count == 2) {
    ...
    actual_count = 0;
    memset(entries, 0, sizeof(entries));
    result = EventGoldcardEntry1_GetAll(manager, entries, 10, &actual_count);
    ...
    if (actual_count > 0) {
        ...
        EventGoldcardEntry1_PrintInfo(&entries[0]);
