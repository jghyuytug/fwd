#include "power_war_guild_rank_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int PowerWarGuildRank_Add(DBConnectionManager* manager, const PowerWarGuildRank* record) {
    ...
    snprintf(query, sizeof(query),
        ...
        "VALUES (%d, %d, %d, %d, %d)",
        record->server_id, record->rank, record->guild_id, record->power_war_point, record->power_side);
...
int PowerWarGuildRank_Get(DBConnectionManager* manager, int guild_id, PowerWarGuildRank* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT server_id, rank, guild_id, power_war_point, power_side FROM power_war_guild_rank WHERE guild_id = %d",
        guild_id);
    ...
    memset(record, 0, sizeof(PowerWarGuildRank));
...
int PowerWarGuildRank_GetAll(DBConnectionManager* manager, PowerWarGuildRank* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT server_id, rank, guild_id, power_war_point, power_side FROM power_war_guild_rank");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_GUILD, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(PowerWarGuildRank));
