...
int GuildMember_GetByCharacNo(DBConnectionManager* manager, int charac_no, GuildMember* record) {
    ...
    snprintf(query, sizeof(query),
             "SELECT guild_id, m_id, server_id, charac_no, charac_name, memo, grade, job, grow_type, lev, age, born_year, sex, apply_time, member_time, member_flag, bbs_cnt, last_visit_time, secede_type, secede_time, member_point, member_point_prev, last_play_time, nick_name "
             "FROM guild_member WHERE charac_no = %d AND member_flag = 1 LIMIT 1",
             charac_no);
...
int GuildMember_GetByGuild(DBConnectionManager* manager, int guild_id, GuildMember* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
             "SELECT guild_id, m_id, server_id, charac_no, charac_name, memo, grade, job, grow_type, lev, age, born_year, sex, apply_time, member_time, member_flag, bbs_cnt, last_visit_time, secede_type, secede_time, member_point, member_point_prev, last_play_time, nick_name "
             "FROM guild_member WHERE guild_id = %d AND member_flag = 1 "
