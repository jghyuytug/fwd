...
int DblabDbConnect130516_GetAll(DBConnectionManager* manager, DblabDbConnect130516* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, host_name, db_server_group, db_type, db_name, db_ip, db_port, db_userid, db_passwd, comments FROM dblab_db_connect_130516");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_TW, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(DblabDbConnect130516));
