...
int MemberPunishInfoHistory_Add(DBConnectionManager* manager, const MemberPunishInfoHistory* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO member_punish_info_history (m_id, punish_type, occ_time, punish_value, apply_flag, start_time, end_time, admin_id, reason, is_kicked, first_ssn, second_ssn) "
...
int MemberPunishInfoHistory_Get(DBConnectionManager* manager, int no, MemberPunishInfoHistory* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, m_id, punish_type, occ_time, punish_value, apply_flag, start_time, end_time, admin_id, reason, is_kicked, first_ssn, second_ssn FROM member_punish_info_history WHERE no = %d",
        no);
...
int MemberPunishInfoHistory_GetAll(DBConnectionManager* manager, MemberPunishInfoHistory* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, m_id, punish_type, occ_time, punish_value, apply_flag, start_time, end_time, admin_id, reason, is_kicked, first_ssn, second_ssn FROM member_punish_info_history");
