...
int MemberInfo_Add(DBConnectionManager* manager, const MemberInfo* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO member_info (user_id, user_name, first_ssn, second_ssn, passwd, mobile_no, reg_date, email, q_no, q_answer, updt_date, state, nickname, email_yn, ssn_check, slot, last_play_time, hangame_flag, hanmon_flag, m_type) "
...
int MemberInfo_Get(DBConnectionManager* manager, int m_id, MemberInfo* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT m_id, user_id, user_name, first_ssn, second_ssn, passwd, mobile_no, reg_date, email, q_no, q_answer, updt_date, state, nickname, email_yn, ssn_check, slot, last_play_time, hangame_flag, hanmon_flag, m_type FROM member_info WHERE m_id = %d",
        m_id);
    ...
    memset(record, 0, sizeof(MemberInfo));
    ...
    record->m_type = row[20] ? atoi(row[20]) : 0;
...
int MemberInfo_GetAll(DBConnectionManager* manager, MemberInfo* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT m_id, user_id, user_name, first_ssn, second_ssn, passwd, mobile_no, reg_date, email, q_no, q_answer, updt_date, state, nickname, email_yn, ssn_check, slot, last_play_time, hangame_flag, hanmon_flag, m_type FROM member_info");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_TW, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(MemberInfo));
        ...
        records[count].m_type = row[20] ? atoi(row[20]) : 0;
        count++;
    }
