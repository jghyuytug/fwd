...
int MemberMiles_Add(DBConnectionManager* manager, const MemberMiles* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO member_miles (m_id, miles, daily_miles) "
        "VALUES (%d, %d, %d)",
...
int MemberMiles_Get(DBConnectionManager* manager, int m_id, MemberMiles* record) {
    ...
    memset(record, 0, sizeof(MemberMiles));
    record->m_id = row[0] ? atoi(row[0]) : 0;
    record->miles = row[1] ? atoi(row[1]) : 0;
    record->daily_miles = row[2] ? atoi(row[2]) : 0;
...
int MemberMiles_GetAll(DBConnectionManager* manager, MemberMiles* records, int max_count, int* actual_count) {
    ...
    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(MemberMiles));
        records[count].m_id = row[0] ? atoi(row[0]) : 0;
        records[count].miles = row[1] ? atoi(row[1]) : 0;
        records[count].daily_miles = row[2] ? atoi(row[2]) : 0;
        count++;
    }
