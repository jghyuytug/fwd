...
int MemberInfoUtf8_Add(DBConnectionManager* manager, const MemberInfoUtf8* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO member_info_utf8 (m_id, user_id, user_name, first_ssn, second_ssn, passwd, mobile_no, reg_date, email, q_no, q_answer, updt_date, state, nickname, email_yn, ssn_check, slot, last_play_time, hangame_flag, hanmon_flag) "
        "VALUES (%d, '%s', '%s', '%s', '%s', '%s', '%s', %d, '%s', %d, '%s', '%s', %d, '%s', %d, %d, %d, '%s', %d, %d)",
        record->m_id, record->user_id, record->user_name, record->first_ssn, record->second_ssn, record->passwd, record->mobile_no, record->reg_date, record->email, record->q_no, record->q_answer, record->updt_date, record->state, record->nickname, record->email_yn, record->ssn_check, record->slot, record->last_play_time, record->hangame_flag, record->hanmon_flag);
...
int MemberInfoUtf8_Get(DBConnectionManager* manager, int m_id, MemberInfoUtf8* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT m_id, user_id, user_name, first_ssn, second_ssn, passwd, mobile_no, reg_date, email, q_no, q_answer, updt_date, state, nickname, email_yn, ssn_check, slot, last_play_time, hangame_flag, hanmon_flag FROM member_info_utf8 WHERE m_id = %d",
        m_id);
    ...
    memset(record, 0, sizeof(MemberInfoUtf8));
...
int MemberInfoUtf8_GetAll(DBConnectionManager* manager, MemberInfoUtf8* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT m_id, user_id, user_name, first_ssn, second_ssn, passwd, mobile_no, reg_date, email, q_no, q_answer, updt_date, state, nickname, email_yn, ssn_check, slot, last_play_time, hangame_flag, hanmon_flag FROM member_info_utf8");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_TW, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(MemberInfoUtf8));
