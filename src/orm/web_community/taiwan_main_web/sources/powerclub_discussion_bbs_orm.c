#include "powerclub_discussion_bbs_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int PowerclubDiscussionBbs_Add(DBConnectionManager* manager, const PowerclubDiscussionBbs* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO powerclub_discussion_bbs (notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view) "
        "VALUES (%d, %d, '%s', %d, '%s', %d, %d, %d, '%s', %d, '%s', %d, %d, %d)",
        record->notice, record->category, record->m_nickname, record->m_id, record->m_user_id, record->m_sex, record->server_id, record->charac_no, record->charac_name, record->job, record->title, record->create_day, record->comment, record->view);
...
int PowerclubDiscussionBbs_Get(DBConnectionManager* manager, int no, PowerclubDiscussionBbs* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view FROM powerclub_discussion_bbs WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(PowerclubDiscussionBbs));
...
int PowerclubDiscussionBbs_GetAll(DBConnectionManager* manager, PowerclubDiscussionBbs* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view FROM powerclub_discussion_bbs");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(PowerclubDiscussionBbs));
