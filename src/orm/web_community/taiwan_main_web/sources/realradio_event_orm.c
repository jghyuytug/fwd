...
int RealradioEvent_Add(DBConnectionManager* manager, const RealradioEvent* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO realradio_event (bbs_type, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, adorn, adorn_color1, adorn_color2, file, depth, sequence, open_flag) "
        "VALUES (%d, %d, %d, '%s', %d, '%s', %d, %d, %d, '%s', %d, '%s', %d, %d, %d, %d, %d, %d, %d, %d, %d, %f, %d)",
        record->bbs_type, record->notice, record->category, record->m_nickname, record->m_id, record->m_user_id, record->m_sex, record->server_id, record->charac_no, record->charac_name, record->job, record->title, record->create_day, record->comment, record->view, record->recom, record->adorn, record->adorn_color1, record->adorn_color2, record->file, record->depth, record->sequence, record->open_flag);
...
int RealradioEvent_Get(DBConnectionManager* manager, int no, RealradioEvent* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, bbs_type, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, adorn, adorn_color1, adorn_color2, file, depth, sequence, open_flag FROM realradio_event WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(RealradioEvent));
    record->no = row[0] ? atoi(row[0]) : 0;
    record->bbs_type = row[1] ? atoi(row[1]) : 0;
    ...
    record->open_flag = row[23] ? atoi(row[23]) : 0;
...
int RealradioEvent_GetAll(DBConnectionManager* manager, RealradioEvent* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, bbs_type, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, adorn, adorn_color1, adorn_color2, file, depth, sequence, open_flag FROM realradio_event");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(RealradioEvent));
        ...
        records[count].open_flag = row[23] ? atoi(row[23]) : 0;
        count++;
    }
