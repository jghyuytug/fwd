#include "community_discussion_content_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int CommunityDiscussionContent_Add(DBConnectionManager* manager, const CommunityDiscussionContent* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO community_discussion_content (no, content, content_type, ip) "
        "VALUES (%d, '%s', '%s', '%s')",
...
int CommunityDiscussionContent_Get(DBConnectionManager* manager, int no, CommunityDiscussionContent* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip FROM community_discussion_content WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(CommunityDiscussionContent));
    record->no = row[0] ? atoi(row[0]) : 0;
    if (row[1]) strncpy(record->content, row[1], sizeof(record->content) - 1);
    if (row[2]) strncpy(record->content_type, row[2], sizeof(record->content_type) - 1);
...
int CommunityDiscussionContent_GetAll(DBConnectionManager* manager, CommunityDiscussionContent* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip FROM community_discussion_content");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(CommunityDiscussionContent));
        records[count].no = row[0] ? atoi(row[0]) : 0;
        if (row[1]) strncpy(records[count].content, row[1], sizeof(records[count].content) - 1);
        if (row[2]) strncpy(records[count].content_type, row[2], sizeof(records[count].content_type) - 1);
