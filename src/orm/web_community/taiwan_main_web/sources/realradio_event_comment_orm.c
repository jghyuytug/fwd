#include "realradio_event_comment_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int RealradioEventComment_Add(DBConnectionManager* manager, const RealradioEventComment* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO realradio_event_comment (content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day) "
...
int RealradioEventComment_Get(DBConnectionManager* manager, int no, RealradioEventComment* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day FROM realradio_event_comment WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(RealradioEventComment));
...
int RealradioEventComment_GetAll(DBConnectionManager* manager, RealradioEventComment* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day FROM realradio_event_comment");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(RealradioEventComment));
