...
int GenBbs_Add(DBConnectionManager* manager, const GenBbs* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO gen_bbs (bd_id, empyn, mgno, open, main, reg_date, mod_date, hits, body_type, m_id, reg_id, subject, cs_flag, requestor, order_flag) "
        "VALUES (%d, %d, %d, %d, %d, %d, %d, %d, '%s', %d, '%s', '%s', %d, '%s', %d)",
        record->bd_id, record->empyn, record->mgno, record->open, record->main, record->reg_date, record->mod_date, record->hits, record->body_type, record->m_id, record->reg_id, record->subject, record->cs_flag, record->requestor, record->order_flag);
...
int GenBbs_Get(DBConnectionManager* manager, int gno, GenBbs* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT gno, bd_id, empyn, mgno, open, main, reg_date, mod_date, hits, body_type, m_id, reg_id, subject, cs_flag, requestor, order_flag FROM gen_bbs WHERE gno = %d",
        gno);
    ...
    record->cs_flag = row[13] ? atoi(row[13]) : 0;
    if (row[14]) strncpy(record->requestor, row[14], sizeof(record->requestor) - 1);
    record->order_flag = row[15] ? atoi(row[15]) : 0;
...
int GenBbs_GetAll(DBConnectionManager* manager, GenBbs* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT gno, bd_id, empyn, mgno, open, main, reg_date, mod_date, hits, body_type, m_id, reg_id, subject, cs_flag, requestor, order_flag FROM gen_bbs");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(GenBbs));
        ...
        records[count].cs_flag = row[13] ? atoi(row[13]) : 0;
        if (row[14]) strncpy(records[count].requestor, row[14], sizeof(records[count].requestor) - 1);
        records[count].order_flag = row[15] ? atoi(row[15]) : 0;
        count++;
    }
