...
int IssueOfWeek_Add(DBConnectionManager* manager, const IssueOfWeek* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO issue_of_week (occ_date, title, contents, dnf_main, radio_url, opt_flag, view_cnt) "
        "VALUES ('%s', '%s', '%s', '%s', '%s', %d, %d)",
        record->occ_date, record->title, record->contents, record->dnf_main, record->radio_url, record->opt_flag, record->view_cnt);
...
int IssueOfWeek_Get(DBConnectionManager* manager, int no, IssueOfWeek* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, occ_date, title, contents, dnf_main, radio_url, opt_flag, view_cnt FROM issue_of_week WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(IssueOfWeek));
    ...
    if (row[5]) strncpy(record->radio_url, row[5], sizeof(record->radio_url) - 1);
    record->opt_flag = row[6] ? atoi(row[6]) : 0;
    record->view_cnt = row[7] ? atoi(row[7]) : 0;
...
int IssueOfWeek_GetAll(DBConnectionManager* manager, IssueOfWeek* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, occ_date, title, contents, dnf_main, radio_url, opt_flag, view_cnt FROM issue_of_week");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(IssueOfWeek));
        ...
        if (row[5]) strncpy(records[count].radio_url, row[5], sizeof(records[count].radio_url) - 1);
        records[count].opt_flag = row[6] ? atoi(row[6]) : 0;
        records[count].view_cnt = row[7] ? atoi(row[7]) : 0;
        count++;
    }
