...
int TcgPublicInfo_Add(DBConnectionManager* manager, const TcgPublicInfo* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO tcg_public_info (notice, category, m_id, m_sex, charac_no, charac_name, job, title, create_day, comment, view, file) "
...
int TcgPublicInfo_Get(DBConnectionManager* manager, int no, TcgPublicInfo* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, notice, category, m_id, m_sex, charac_no, charac_name, job, title, create_day, comment, view, file FROM tcg_public_info WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(TcgPublicInfo));
...
int TcgPublicInfo_GetAll(DBConnectionManager* manager, TcgPublicInfo* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, notice, category, m_id, m_sex, charac_no, charac_name, job, title, create_day, comment, view, file FROM tcg_public_info");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(TcgPublicInfo));
