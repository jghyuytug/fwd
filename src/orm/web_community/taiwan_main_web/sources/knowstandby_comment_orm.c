#include "knowstandby_comment_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int KnowstandbyComment_Add(DBConnectionManager* manager, const KnowstandbyComment* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO knowstandby_comment (content_no, m_id, server_id, charac_no, charac_name, job, comment_text, ip, create_day, value) "
...
int KnowstandbyComment_Get(DBConnectionManager* manager, int no, KnowstandbyComment* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_id, server_id, charac_no, charac_name, job, comment_text, ip, create_day, value FROM knowstandby_comment WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(KnowstandbyComment));
...
int KnowstandbyComment_GetAll(DBConnectionManager* manager, KnowstandbyComment* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_id, server_id, charac_no, charac_name, job, comment_text, ip, create_day, value FROM knowstandby_comment");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(KnowstandbyComment));
