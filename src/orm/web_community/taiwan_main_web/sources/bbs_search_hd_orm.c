...
int BbsSearchHd_Add(DBConnectionManager* manager, const BbsSearchHd* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO bbs_search_hd (bbs_class, f_key, s_key, subject, body, reg_date) "
        "VALUES (%d, %d, %d, '%s', '%s', %d)",
...
int BbsSearchHd_GetAll(DBConnectionManager* manager, BbsSearchHd* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT bbs_class, f_key, s_key, subject, body, reg_date FROM bbs_search_hd");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(BbsSearchHd));
