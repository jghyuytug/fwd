...
int OtherBbsinfo2_Add(DBConnectionManager* manager, const OtherBbsinfo2* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO other_bbsinfo2 (kn_id, url_go, url_to, hits, reg_date, subject, ref_url, content) "
...
int OtherBbsinfo2_Get(DBConnectionManager* manager, const char* url_go, const char* url_to, OtherBbsinfo2* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT kn_id, url_go, url_to, hits, reg_date, subject, ref_url, content FROM other_bbsinfo2 WHERE url_go = '%s' AND url_to = '%s'",
        url_go, url_to);
    ...
    memset(record, 0, sizeof(OtherBbsinfo2));
...
int OtherBbsinfo2_GetAll(DBConnectionManager* manager, OtherBbsinfo2* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT kn_id, url_go, url_to, hits, reg_date, subject, ref_url, content FROM other_bbsinfo2");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(OtherBbsinfo2));
