...
int Realradio_Add(DBConnectionManager* manager, const Realradio* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO realradio (bbs_type, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, adorn, adorn_color1, adorn_color2, file, depth, sequence) "
        "VALUES (%d, %d, %d, '%s', %d, '%s', %d, %d, %d, '%s', %d, '%s', %d, %d, %d, %d, %d, %d, %d, %d, %d, %f)",
...
int Realradio_Get(DBConnectionManager* manager, int no, Realradio* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, bbs_type, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, adorn, adorn_color1, adorn_color2, file, depth, sequence FROM realradio WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(Realradio));
...
int Realradio_GetAll(DBConnectionManager* manager, Realradio* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, bbs_type, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, adorn, adorn_color1, adorn_color2, file, depth, sequence FROM realradio");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(Realradio));
