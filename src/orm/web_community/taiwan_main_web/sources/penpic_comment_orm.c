...
int PenpicComment_Add(DBConnectionManager* manager, const PenpicComment* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO penpic_comment (content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, point, ip, create_day) "
...
int PenpicComment_Get(DBConnectionManager* manager, int no, PenpicComment* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, point, ip, create_day FROM penpic_comment WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(PenpicComment));
...
int PenpicComment_GetAll(DBConnectionManager* manager, PenpicComment* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, point, ip, create_day FROM penpic_comment");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(PenpicComment));
