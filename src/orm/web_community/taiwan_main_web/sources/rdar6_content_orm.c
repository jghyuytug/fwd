...
int Rdar6Content_Add(DBConnectionManager* manager, const Rdar6Content* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO rdar6_content (no, content, content_type, ip, ring, sms) "
        "VALUES (%d, '%s', %d, '%s', %d, %d)",
...
int Rdar6Content_Get(DBConnectionManager* manager, int no, Rdar6Content* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM rdar6_content WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(Rdar6Content));
...
int Rdar6Content_GetAll(DBConnectionManager* manager, Rdar6Content* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM rdar6_content");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(Rdar6Content));
