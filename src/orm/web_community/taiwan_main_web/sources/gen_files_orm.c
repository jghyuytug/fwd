...
int GenFiles_Add(DBConnectionManager* manager, const GenFiles* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO gen_files (gno, file_server, file_location) "
        "VALUES (%d, '%s', '%s')",
...
int GenFiles_Get(DBConnectionManager* manager, int gno, signed char gf_no, GenFiles* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT gno, gf_no, file_server, file_location FROM gen_files WHERE gno = %d AND gf_no = %d",
        gno, gf_no);
    ...
    memset(record, 0, sizeof(GenFiles));
...
int GenFiles_GetAll(DBConnectionManager* manager, GenFiles* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT gno, gf_no, file_server, file_location FROM gen_files");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(GenFiles));
