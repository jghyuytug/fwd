#include "intensive_discussion_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int IntensiveDiscussion_Add(DBConnectionManager* manager, const IntensiveDiscussion* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO intensive_discussion (notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, file, depth, sequence, positive, negative) "
        "VALUES (%d, %d, '%s', %d, '%s', %d, %d, %d, '%s', %d, '%s', %d, %d, %d, %d, %d, %d, %f, %d, %d)",
        record->notice, record->category, record->m_nickname, record->m_id, record->m_user_id, record->m_sex, record->server_id, record->charac_no, record->charac_name, record->job, record->title, record->create_day, record->comment, record->view, record->recom, record->file, record->depth, record->sequence, record->positive, record->negative);
...
int IntensiveDiscussion_Get(DBConnectionManager* manager, int no, IntensiveDiscussion* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, file, depth, sequence, positive, negative FROM intensive_discussion WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(IntensiveDiscussion));
...
int IntensiveDiscussion_GetAll(DBConnectionManager* manager, IntensiveDiscussion* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, file, depth, sequence, positive, negative FROM intensive_discussion");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(IntensiveDiscussion));
