...
int KnowledgeClass_Add(DBConnectionManager* manager, const KnowledgeClass* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO knowledge_class (kn_id, sup_id, kn_name, cur_depth) "
        "VALUES (%d, %d, '%s', %d)",
...
int KnowledgeClass_Get(DBConnectionManager* manager, long long kn_id, KnowledgeClass* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT kn_id, sup_id, kn_name, cur_depth FROM knowledge_class WHERE kn_id = %d",
        kn_id);
    ...
    memset(record, 0, sizeof(KnowledgeClass));
    record->kn_id = row[0] ? atoll(row[0]) : 0;
    record->sup_id = row[1] ? atoll(row[1]) : 0;
...
int KnowledgeClass_GetAll(DBConnectionManager* manager, KnowledgeClass* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT kn_id, sup_id, kn_name, cur_depth FROM knowledge_class");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(KnowledgeClass));
        records[count].kn_id = row[0] ? atoll(row[0]) : 0;
        records[count].sup_id = row[1] ? atoll(row[1]) : 0;
