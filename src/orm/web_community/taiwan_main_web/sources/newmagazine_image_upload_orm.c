#include "newmagazine_image_upload_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int NewmagazineImageUpload_Add(DBConnectionManager* manager, const NewmagazineImageUpload* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO newmagazine_image_upload (date_start, date_end, img_url) "
        "VALUES ('%s', '%s', '%s')",
...
int NewmagazineImageUpload_Get(DBConnectionManager* manager, int no, NewmagazineImageUpload* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, date_start, date_end, img_url FROM newmagazine_image_upload WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(NewmagazineImageUpload));
...
int NewmagazineImageUpload_GetAll(DBConnectionManager* manager, NewmagazineImageUpload* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, date_start, date_end, img_url FROM newmagazine_image_upload");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(NewmagazineImageUpload));
