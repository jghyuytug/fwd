#include "powerclub_discussion_comment_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int PowerclubDiscussionComment_Add(DBConnectionManager* manager, const PowerclubDiscussionComment* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO powerclub_discussion_comment (content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day, recom, opinion_mode, del_flag, comment_cnt) "
        "VALUES (%d, '%s', %d, '%s', %d, %d, %d, '%s', %d, '%s', '%s', %d, %d, %d, %d, %d)",
        record->content_no, record->m_nickname, record->m_id, record->m_user_id, record->m_sex, record->server_id, record->charac_no, record->charac_name, record->job, record->comment_text, record->ip, record->create_day, record->recom, record->opinion_mode, record->del_flag, record->comment_cnt);
...
int PowerclubDiscussionComment_Get(DBConnectionManager* manager, int no, PowerclubDiscussionComment* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day, recom, opinion_mode, del_flag, comment_cnt FROM powerclub_discussion_comment WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(PowerclubDiscussionComment));
    ...
    record->opinion_mode = row[14] ? atoi(row[14]) : 0;
    record->del_flag = row[15] ? atoi(row[15]) : 0;
    record->comment_cnt = row[16] ? atoi(row[16]) : 0;
...
int PowerclubDiscussionComment_GetAll(DBConnectionManager* manager, PowerclubDiscussionComment* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day, recom, opinion_mode, del_flag, comment_cnt FROM powerclub_discussion_comment");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(PowerclubDiscussionComment));
        ...
        records[count].opinion_mode = row[14] ? atoi(row[14]) : 0;
        records[count].del_flag = row[15] ? atoi(row[15]) : 0;
        records[count].comment_cnt = row[16] ? atoi(row[16]) : 0;
        count++;
    }
