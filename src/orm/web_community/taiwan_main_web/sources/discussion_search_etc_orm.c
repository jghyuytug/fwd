#include "discussion_search_etc_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int DiscussionSearchEtc_Add(DBConnectionManager* manager, const DiscussionSearchEtc* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO discussion_search_etc (no, category, class, word) "
        "VALUES (%d, %d, %d, '%s')",
...
int DiscussionSearchEtc_GetAll(DBConnectionManager* manager, DiscussionSearchEtc* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, category, class, word FROM discussion_search_etc");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(DiscussionSearchEtc));
