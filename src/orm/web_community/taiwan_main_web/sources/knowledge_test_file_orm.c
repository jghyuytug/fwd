#include "knowledge_test_file_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int KnowledgeTestFile_Add(DBConnectionManager* manager, const KnowledgeTestFile* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO knowledge_test_file (content_no, file_url) "
        "VALUES (%d, '%s')",
...
int KnowledgeTestFile_Get(DBConnectionManager* manager, int no, KnowledgeTestFile* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, file_url FROM knowledge_test_file WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(KnowledgeTestFile));
...
int KnowledgeTestFile_GetAll(DBConnectionManager* manager, KnowledgeTestFile* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, file_url FROM knowledge_test_file");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(KnowledgeTestFile));
