#include "skill_dictionary_comment_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int SkillDictionaryComment_Add(DBConnectionManager* manager, const SkillDictionaryComment* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO skill_dictionary_comment (content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day, skill_group, skill_idx, charac_idx) "
        "VALUES (%d, '%s', %d, '%s', %d, %d, %d, '%s', %d, '%s', '%s', %d, %d, %d, %d)",
        record->content_no, record->m_nickname, record->m_id, record->m_user_id, record->m_sex, record->server_id, record->charac_no, record->charac_name, record->job, record->comment_text, record->ip, record->create_day, record->skill_group, record->skill_idx, record->charac_idx);
...
int SkillDictionaryComment_Get(DBConnectionManager* manager, int no, SkillDictionaryComment* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day, skill_group, skill_idx, charac_idx FROM skill_dictionary_comment WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(SkillDictionaryComment));
    ...
    record->skill_group = row[13] ? atoi(row[13]) : 0;
    record->skill_idx = row[14] ? atoi(row[14]) : 0;
    record->charac_idx = row[15] ? atoi(row[15]) : 0;
...
int SkillDictionaryComment_GetAll(DBConnectionManager* manager, SkillDictionaryComment* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day, skill_group, skill_idx, charac_idx FROM skill_dictionary_comment");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(SkillDictionaryComment));
        ...
        records[count].skill_group = row[13] ? atoi(row[13]) : 0;
        records[count].skill_idx = row[14] ? atoi(row[14]) : 0;
        records[count].charac_idx = row[15] ? atoi(row[15]) : 0;
        count++;
    }
