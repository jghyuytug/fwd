#include "realradio_event_file_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int RealradioEventFile_Add(DBConnectionManager* manager, const RealradioEventFile* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO realradio_event_file (content_no, file_url) "
        "VALUES (%d, '%s')",
...
int RealradioEventFile_Get(DBConnectionManager* manager, int no, RealradioEventFile* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, file_url FROM realradio_event_file WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(RealradioEventFile));
...
int RealradioEventFile_GetAll(DBConnectionManager* manager, RealradioEventFile* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, file_url FROM realradio_event_file");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(RealradioEventFile));
