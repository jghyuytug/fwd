#include "realradio_event_comment_view_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int RealradioEventCommentView_Add(DBConnectionManager* manager, const RealradioEventCommentView* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO realradio_event_comment_view (content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day) "
...
int RealradioEventCommentView_Get(DBConnectionManager* manager, int no, RealradioEventCommentView* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day FROM realradio_event_comment_view WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(RealradioEventCommentView));
...
int RealradioEventCommentView_GetAll(DBConnectionManager* manager, RealradioEventCommentView* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day FROM realradio_event_comment_view");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(RealradioEventCommentView));
