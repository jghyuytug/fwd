#include "powerclub_discussion_bbs_content_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int PowerclubDiscussionBbsContent_Add(DBConnectionManager* manager, const PowerclubDiscussionBbsContent* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO powerclub_discussion_bbs_content (no, content, content_type, ip, ring, sms) "
        "VALUES (%d, '%s', %d, '%s', %d, %d)",
...
int PowerclubDiscussionBbsContent_Get(DBConnectionManager* manager, int no, PowerclubDiscussionBbsContent* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM powerclub_discussion_bbs_content WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(PowerclubDiscussionBbsContent));
...
int PowerclubDiscussionBbsContent_GetAll(DBConnectionManager* manager, PowerclubDiscussionBbsContent* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM powerclub_discussion_bbs_content");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(PowerclubDiscussionBbsContent));
