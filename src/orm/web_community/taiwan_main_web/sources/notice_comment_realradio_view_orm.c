#include "notice_comment_realradio_view_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int NoticeCommentRealradioView_Add(DBConnectionManager* manager, const NoticeCommentRealradioView* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO notice_comment_realradio_view (content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day) "
...
int NoticeCommentRealradioView_Get(DBConnectionManager* manager, int no, NoticeCommentRealradioView* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day FROM notice_comment_realradio_view WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(NoticeCommentRealradioView));
...
int NoticeCommentRealradioView_GetAll(DBConnectionManager* manager, NoticeCommentRealradioView* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day FROM notice_comment_realradio_view");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(NoticeCommentRealradioView));
