...
int Rdar6_Add(DBConnectionManager* manager, const Rdar6* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO rdar6 (notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, adorn, adorn_color1, adorn_color2, file, depth, sequence) "
...
int Rdar6_Get(DBConnectionManager* manager, int no, Rdar6* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, adorn, adorn_color1, adorn_color2, file, depth, sequence FROM rdar6 WHERE no = %d",
        no);
...
int Rdar6_GetAll(DBConnectionManager* manager, Rdar6* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, adorn, adorn_color1, adorn_color2, file, depth, sequence FROM rdar6");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(Rdar6));
