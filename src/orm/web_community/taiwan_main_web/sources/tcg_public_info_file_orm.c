#include "tcg_public_info_file_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int TcgPublicInfoFile_Add(DBConnectionManager* manager, const TcgPublicInfoFile* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO tcg_public_info_file (content_no, file_url) "
        "VALUES (%d, '%s')",
...
int TcgPublicInfoFile_Get(DBConnectionManager* manager, int no, TcgPublicInfoFile* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, file_url FROM tcg_public_info_file WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(TcgPublicInfoFile));
...
int TcgPublicInfoFile_GetAll(DBConnectionManager* manager, TcgPublicInfoFile* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, file_url FROM tcg_public_info_file");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(TcgPublicInfoFile));
