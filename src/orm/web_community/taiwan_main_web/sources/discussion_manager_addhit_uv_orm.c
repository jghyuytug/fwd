#include "discussion_manager_addhit_uv_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int DiscussionManagerAddhitUv_Add(DBConnectionManager* manager, const DiscussionManagerAddhitUv* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO discussion_manager_addhit_uv (click_view) "
        "VALUES (%d)",
        record->click_view);
...
int DiscussionManagerAddhitUv_Get(DBConnectionManager* manager, int no, DiscussionManagerAddhitUv* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, click_view FROM discussion_manager_addhit_uv WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(DiscussionManagerAddhitUv));
...
int DiscussionManagerAddhitUv_GetAll(DBConnectionManager* manager, DiscussionManagerAddhitUv* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, click_view FROM discussion_manager_addhit_uv");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(DiscussionManagerAddhitUv));
