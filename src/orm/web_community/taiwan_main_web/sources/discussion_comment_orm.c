#include "discussion_comment_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int DiscussionComment_Add(DBConnectionManager* manager, const DiscussionComment* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO discussion_comment (content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, comment_text, ip, create_day, modify_time, modify_cnt) "
        "VALUES (%d, '%s', %d, '%s', %d, %d, %d, '%s', %d, '%s', '%s', '%s', %d, %d, %d)",
        record->content_no, record->m_nickname, record->m_id, record->m_user_id, record->m_sex, record->server_id, record->charac_no, record->charac_name, record->job, record->title, record->comment_text, record->ip, record->create_day, record->modify_time, record->modify_cnt);
...
int DiscussionComment_Get(DBConnectionManager* manager, int no, DiscussionComment* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, comment_text, ip, create_day, modify_time, modify_cnt FROM discussion_comment WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(DiscussionComment));
    ...
    if (row[11]) strncpy(record->comment_text, row[11], sizeof(record->comment_text) - 1);
    if (row[12]) strncpy(record->ip, row[12], sizeof(record->ip) - 1);
    record->create_day = row[13] ? atoi(row[13]) : 0;
    record->modify_time = row[14] ? atoi(row[14]) : 0;
    record->modify_cnt = row[15] ? atoi(row[15]) : 0;
...
int DiscussionComment_GetAll(DBConnectionManager* manager, DiscussionComment* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, comment_text, ip, create_day, modify_time, modify_cnt FROM discussion_comment");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(DiscussionComment));
        ...
        if (row[11]) strncpy(records[count].comment_text, row[11], sizeof(records[count].comment_text) - 1);
        if (row[12]) strncpy(records[count].ip, row[12], sizeof(records[count].ip) - 1);
        records[count].create_day = row[13] ? atoi(row[13]) : 0;
        records[count].modify_time = row[14] ? atoi(row[14]) : 0;
        records[count].modify_cnt = row[15] ? atoi(row[15]) : 0;
        count++;
    }
