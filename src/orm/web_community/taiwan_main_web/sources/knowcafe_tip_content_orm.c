#include "knowcafe_tip_content_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int KnowcafeTipContent_Add(DBConnectionManager* manager, const KnowcafeTipContent* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO knowcafe_tip_content (no, content, content_type, ip, ring, sms) "
        "VALUES (%d, '%s', %d, '%s', %d, %d)",
...
int KnowcafeTipContent_Get(DBConnectionManager* manager, int no, KnowcafeTipContent* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM knowcafe_tip_content WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(KnowcafeTipContent));
...
int KnowcafeTipContent_GetAll(DBConnectionManager* manager, KnowcafeTipContent* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM knowcafe_tip_content");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(KnowcafeTipContent));
