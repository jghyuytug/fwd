#include "penpic4_valuation_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int Penpic4Valuation_Add(DBConnectionManager* manager, const Penpic4Valuation* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO penpic4_valuation (no, penpic_desc, penpic_examine) "
        "VALUES (%d, '%s', '%s')",
...
int Penpic4Valuation_Get(DBConnectionManager* manager, int no, Penpic4Valuation* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, penpic_desc, penpic_examine FROM penpic4_valuation WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(Penpic4Valuation));
...
int Penpic4Valuation_GetAll(DBConnectionManager* manager, Penpic4Valuation* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, penpic_desc, penpic_examine FROM penpic4_valuation");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(Penpic4Valuation));
