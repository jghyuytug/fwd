#include "download_library_content_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int DownloadLibraryContent_Add(DBConnectionManager* manager, const DownloadLibraryContent* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO download_library_content (no, content) "
        "VALUES (%d, '%s')",
        record->no, record->content);
...
int DownloadLibraryContent_Get(DBConnectionManager* manager, int no, DownloadLibraryContent* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content FROM download_library_content WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(DownloadLibraryContent));
...
int DownloadLibraryContent_GetAll(DBConnectionManager* manager, DownloadLibraryContent* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content FROM download_library_content");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(DownloadLibraryContent));
