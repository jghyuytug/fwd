#include "realradio_comment_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int RealradioComment_Add(DBConnectionManager* manager, const RealradioComment* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO realradio_comment (content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day, sequence) "
        "VALUES (%d, '%s', %d, '%s', %d, %d, %d, '%s', %d, '%s', '%s', %d, %f)",
        record->content_no, record->m_nickname, record->m_id, record->m_user_id, record->m_sex, record->server_id, record->charac_no, record->charac_name, record->job, record->comment_text, record->ip, record->create_day, record->sequence);
...
int RealradioComment_Get(DBConnectionManager* manager, int no, RealradioComment* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day, sequence FROM realradio_comment WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(RealradioComment));
    ...
    record->sequence = row[13] ? atof(row[13]) : 0;
...
int RealradioComment_GetAll(DBConnectionManager* manager, RealradioComment* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day, sequence FROM realradio_comment");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(RealradioComment));
        ...
        records[count].sequence = row[13] ? atof(row[13]) : 0;
        count++;
    }
