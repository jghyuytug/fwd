#include "powerclub_discussion_search_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int PowerclubDiscussionSearch_Add(DBConnectionManager* manager, const PowerclubDiscussionSearch* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO powerclub_discussion_search (no, category, class, word) "
        "VALUES (%d, %d, %d, '%s')",
...
int PowerclubDiscussionSearch_GetAll(DBConnectionManager* manager, PowerclubDiscussionSearch* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, category, class, word FROM powerclub_discussion_search");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(PowerclubDiscussionSearch));
