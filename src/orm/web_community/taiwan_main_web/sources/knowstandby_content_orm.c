#include "knowstandby_content_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int KnowstandbyContent_Add(DBConnectionManager* manager, const KnowstandbyContent* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO knowstandby_content (no, content, content_type, ip) "
        "VALUES (%d, '%s', %d, '%s')",
...
int KnowstandbyContent_Get(DBConnectionManager* manager, int no, KnowstandbyContent* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip FROM knowstandby_content WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(KnowstandbyContent));
...
int KnowstandbyContent_GetAll(DBConnectionManager* manager, KnowstandbyContent* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip FROM knowstandby_content");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(KnowstandbyContent));
