...
int PenpicContent_Add(DBConnectionManager* manager, const PenpicContent* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO penpic_content (no, content, content_type, ip, ring, sms) "
        "VALUES (%d, '%s', %d, '%s', %d, %d)",
...
int PenpicContent_Get(DBConnectionManager* manager, int no, PenpicContent* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM penpic_content WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(PenpicContent));
...
int PenpicContent_GetAll(DBConnectionManager* manager, PenpicContent* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM penpic_content");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(PenpicContent));
