...
int Penpic2Content_Add(DBConnectionManager* manager, const Penpic2Content* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO penpic2_content (no, content, content_type, ip, ring, sms) "
        "VALUES (%d, '%s', %d, '%s', %d, %d)",
...
int Penpic2Content_Get(DBConnectionManager* manager, int no, Penpic2Content* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM penpic2_content WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(Penpic2Content));
...
int Penpic2Content_GetAll(DBConnectionManager* manager, Penpic2Content* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM penpic2_content");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(Penpic2Content));
