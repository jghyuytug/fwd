#include "tcg_tune_comment_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int TcgTuneComment_Add(DBConnectionManager* manager, const TcgTuneComment* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO tcg_tune_comment (content_no, m_id, charac_no, charac_name, comment_text, ip, create_day) "
...
int TcgTuneComment_Get(DBConnectionManager* manager, int no, TcgTuneComment* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_id, charac_no, charac_name, comment_text, ip, create_day FROM tcg_tune_comment WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(TcgTuneComment));
...
int TcgTuneComment_GetAll(DBConnectionManager* manager, TcgTuneComment* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_id, charac_no, charac_name, comment_text, ip, create_day FROM tcg_tune_comment");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(TcgTuneComment));
