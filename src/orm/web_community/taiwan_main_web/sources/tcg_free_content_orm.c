#include "tcg_free_content_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int TcgFreeContent_Add(DBConnectionManager* manager, const TcgFreeContent* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO tcg_free_content (no, content, content_type, ip, ring, sms) "
        "VALUES (%d, '%s', %d, '%s', %d, %d)",
...
int TcgFreeContent_Get(DBConnectionManager* manager, int no, TcgFreeContent* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM tcg_free_content WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(TcgFreeContent));
...
int TcgFreeContent_GetAll(DBConnectionManager* manager, TcgFreeContent* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM tcg_free_content");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(TcgFreeContent));
