#include "discussion_entry_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int DiscussionEntry_Add(DBConnectionManager* manager, const DiscussionEntry* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO discussion_entry (content_no, m_id) "
        "VALUES (%d, %d)",
        record->content_no, record->m_id);
...
int DiscussionEntry_Get(DBConnectionManager* manager, int content_no, int m_id, DiscussionEntry* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT content_no, m_id FROM discussion_entry WHERE content_no = %d AND m_id = %d",
        content_no, m_id);
    ...
    memset(record, 0, sizeof(DiscussionEntry));
...
int DiscussionEntry_GetAll(DBConnectionManager* manager, DiscussionEntry* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT content_no, m_id FROM discussion_entry");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(DiscussionEntry));
