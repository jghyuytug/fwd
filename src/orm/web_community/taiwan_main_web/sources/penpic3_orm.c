...
int Penpic3_Add(DBConnectionManager* manager, const Penpic3* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO penpic3 (penpic_type, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, point, adorn, adorn_color1, adorn_color2, degree) "
        "VALUES (%d, %d, %d, '%s', %d, '%s', %d, %d, %d, '%s', %d, '%s', %d, %d, %d, %d, %d, %d, %d, %d, %d)",
        record->penpic_type, record->notice, record->category, record->m_nickname, record->m_id, record->m_user_id, record->m_sex, record->server_id, record->charac_no, record->charac_name, record->job, record->title, record->create_day, record->comment, record->view, record->recom, record->point, record->adorn, record->adorn_color1, record->adorn_color2, record->degree);
...
int Penpic3_Get(DBConnectionManager* manager, int no, Penpic3* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, penpic_type, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, point, adorn, adorn_color1, adorn_color2, degree FROM penpic3 WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(Penpic3));
    ...
    record->degree = row[21] ? atoi(row[21]) : 0;
...
int Penpic3_GetAll(DBConnectionManager* manager, Penpic3* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, penpic_type, notice, category, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, title, create_day, comment, view, recom, point, adorn, adorn_color1, adorn_color2, degree FROM penpic3");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(Penpic3));
        ...
        records[count].degree = row[21] ? atoi(row[21]) : 0;
        count++;
    }
