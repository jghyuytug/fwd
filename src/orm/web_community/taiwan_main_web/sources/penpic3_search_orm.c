...
int Penpic3Search_Add(DBConnectionManager* manager, const Penpic3Search* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO penpic3_search (no, penpic_type, category, class, word, degree) "
        "VALUES (%d, %d, %d, %d, '%s', %d)",
...
int Penpic3Search_GetAll(DBConnectionManager* manager, Penpic3Search* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, penpic_type, category, class, word, degree FROM penpic3_search");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(Penpic3Search));
