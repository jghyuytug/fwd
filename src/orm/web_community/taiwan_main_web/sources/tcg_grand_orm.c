...
int TcgGrand_Add(DBConnectionManager* manager, const TcgGrand* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO tcg_grand (notice, category, m_id, m_sex, charac_no, charac_name, job, title, create_day, comment, view, file) "
...
int TcgGrand_Get(DBConnectionManager* manager, int no, TcgGrand* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, notice, category, m_id, m_sex, charac_no, charac_name, job, title, create_day, comment, view, file FROM tcg_grand WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(TcgGrand));
...
int TcgGrand_GetAll(DBConnectionManager* manager, TcgGrand* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, notice, category, m_id, m_sex, charac_no, charac_name, job, title, create_day, comment, view, file FROM tcg_grand");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(TcgGrand));
