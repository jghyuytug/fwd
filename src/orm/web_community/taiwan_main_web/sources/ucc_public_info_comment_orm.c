#include "ucc_public_info_comment_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int UccPublicInfoComment_Add(DBConnectionManager* manager, const UccPublicInfoComment* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO ucc_public_info_comment (content_no, m_id, charac_no, charac_name, comment_text, ip, create_day) "
...
int UccPublicInfoComment_Get(DBConnectionManager* manager, int no, UccPublicInfoComment* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_id, charac_no, charac_name, comment_text, ip, create_day FROM ucc_public_info_comment WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(UccPublicInfoComment));
...
int UccPublicInfoComment_GetAll(DBConnectionManager* manager, UccPublicInfoComment* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_id, charac_no, charac_name, comment_text, ip, create_day FROM ucc_public_info_comment");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(UccPublicInfoComment));
