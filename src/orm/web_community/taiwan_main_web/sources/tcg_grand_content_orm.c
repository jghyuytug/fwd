#include "tcg_grand_content_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int TcgGrandContent_Add(DBConnectionManager* manager, const TcgGrandContent* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO tcg_grand_content (no, content, content_type, ip, ring, sms) "
        "VALUES (%d, '%s', %d, '%s', %d, %d)",
...
int TcgGrandContent_Get(DBConnectionManager* manager, int no, TcgGrandContent* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM tcg_grand_content WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(TcgGrandContent));
...
int TcgGrandContent_GetAll(DBConnectionManager* manager, TcgGrandContent* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content, content_type, ip, ring, sms FROM tcg_grand_content");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(TcgGrandContent));
