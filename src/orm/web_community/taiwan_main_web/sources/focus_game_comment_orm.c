#include "focus_game_comment_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int FocusGameComment_Add(DBConnectionManager* manager, const FocusGameComment* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO focus_game_comment (content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day) "
...
int FocusGameComment_Get(DBConnectionManager* manager, int no, FocusGameComment* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day FROM focus_game_comment WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(FocusGameComment));
...
int FocusGameComment_GetAll(DBConnectionManager* manager, FocusGameComment* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day FROM focus_game_comment");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(FocusGameComment));
