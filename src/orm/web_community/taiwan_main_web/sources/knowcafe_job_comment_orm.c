#include "knowcafe_job_comment_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int KnowcafeJobComment_Add(DBConnectionManager* manager, const KnowcafeJobComment* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO knowcafe_job_comment (content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day) "
...
int KnowcafeJobComment_Get(DBConnectionManager* manager, int no, KnowcafeJobComment* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day FROM knowcafe_job_comment WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(KnowcafeJobComment));
...
int KnowcafeJobComment_GetAll(DBConnectionManager* manager, KnowcafeJobComment* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, content_no, m_nickname, m_id, m_user_id, m_sex, server_id, charac_no, charac_name, job, comment_text, ip, create_day FROM knowcafe_job_comment");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(KnowcafeJobComment));
