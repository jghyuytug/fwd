#include "powerclub_discussion_bbs_search_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int PowerclubDiscussionBbsSearch_Add(DBConnectionManager* manager, const PowerclubDiscussionBbsSearch* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO powerclub_discussion_bbs_search (no, category, class, word) "
        "VALUES (%d, %d, %d, '%s')",
...
int PowerclubDiscussionBbsSearch_GetAll(DBConnectionManager* manager, PowerclubDiscussionBbsSearch* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, category, class, word FROM powerclub_discussion_bbs_search");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_MAIN_WEB, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(PowerclubDiscussionBbsSearch));
