#include "charac_housing_mod_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int CharacHousingMod_Add(DBConnectionManager* manager, const CharacHousingMod* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO charac_housing_mod (server_info, charac_no, installed_old, installed_new, MNG_user_id, work_time, work_type, memo) "
        "VALUES (%d, %d, %d, %d, '%s', '%s', %d, '%s')",
        record->server_info, record->charac_no, record->installed_old, record->installed_new, record->MNG_user_id, record->work_time, record->work_type, record->memo);
...
int CharacHousingMod_Get(DBConnectionManager* manager, int id, CharacHousingMod* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT id, server_info, charac_no, installed_old, installed_new, MNG_user_id, work_time, work_type, memo FROM charac_housing_mod WHERE id = %d",
        id);
    ...
    memset(record, 0, sizeof(CharacHousingMod));
    record->id = row[0] ? atoi(row[0]) : 0;
    record->server_info = row[1] ? atoi(row[1]) : 0;
    record->charac_no = row[2] ? atoi(row[2]) : 0;
    record->installed_old = row[3] ? atoi(row[3]) : 0;
    record->installed_new = row[4] ? atoi(row[4]) : 0;
...
int CharacHousingMod_GetAll(DBConnectionManager* manager, CharacHousingMod* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT id, server_info, charac_no, installed_old, installed_new, MNG_user_id, work_time, work_type, memo FROM charac_housing_mod");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_TAIWAN, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(CharacHousingMod));
        ...
        records[count].installed_old = row[3] ? atoi(row[3]) : 0;
        records[count].installed_new = row[4] ? atoi(row[4]) : 0;
