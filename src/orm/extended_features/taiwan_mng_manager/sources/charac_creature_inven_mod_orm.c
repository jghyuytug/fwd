#include "charac_creature_inven_mod_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int CharacCreatureInvenMod_Add(DBConnectionManager* manager, const CharacCreatureInvenMod* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO charac_creature_inven_mod (server_info, charac_no, MNG_user_id, work_time, work_type, memo) "
        "VALUES (%d, %d, '%s', '%s', %d, '%s')",
...
int CharacCreatureInvenMod_Get(DBConnectionManager* manager, int id, CharacCreatureInvenMod* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT id, server_info, charac_no, MNG_user_id, work_time, work_type, memo FROM charac_creature_inven_mod WHERE id = %d",
        id);
    ...
    memset(record, 0, sizeof(CharacCreatureInvenMod));
...
int CharacCreatureInvenMod_GetAll(DBConnectionManager* manager, CharacCreatureInvenMod* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT id, server_info, charac_no, MNG_user_id, work_time, work_type, memo FROM charac_creature_inven_mod");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_TAIWAN, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(CharacCreatureInvenMod));
