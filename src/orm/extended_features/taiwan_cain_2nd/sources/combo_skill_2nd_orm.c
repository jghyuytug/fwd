...
int ComboSkill2Nd_Add(DBConnectionManager* manager, const ComboSkill2Nd* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO combo_skill_2nd (charac_no, combo_idx, value1, value2, value3, value4, value5, value6) "
...
int ComboSkill2Nd_Get(DBConnectionManager* manager, int charac_no, int combo_idx, ComboSkill2Nd* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT charac_no, combo_idx, value1, value2, value3, value4, value5, value6 FROM combo_skill_2nd WHERE charac_no = %d AND combo_idx = %d",
        charac_no, combo_idx);
    ...
    memset(record, 0, sizeof(ComboSkill2Nd));
...
int ComboSkill2Nd_GetAll(DBConnectionManager* manager, ComboSkill2Nd* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT charac_no, combo_idx, value1, value2, value3, value4, value5, value6 FROM combo_skill_2nd");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_CAIN, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(ComboSkill2Nd));
