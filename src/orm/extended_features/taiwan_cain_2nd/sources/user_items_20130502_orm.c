#include "user_items_20130502_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int UserItems20130502_Add(DBConnectionManager* manager, const UserItems20130502* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO user_items_20130502 (charac_no, slot, it_id, expire_date, obtain_from, reg_date, ipg_agency_no, ability_no, stat, clear_avatar_id, item_lock_key, to_ipg_agency_no, m_time, hidden_option, emblem_endurance, color1, color2, trade_restrict) "
...
int UserItems20130502_Get(DBConnectionManager* manager, int ui_id, UserItems20130502* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT ui_id, charac_no, slot, it_id, expire_date, obtain_from, reg_date, ipg_agency_no, ability_no, stat, clear_avatar_id, item_lock_key, to_ipg_agency_no, m_time, hidden_option, emblem_endurance, color1, color2, trade_restrict FROM user_items_20130502 WHERE ui_id = %d",
        ui_id);
    ...
    memset(record, 0, sizeof(UserItems20130502));
...
int UserItems20130502_GetAll(DBConnectionManager* manager, UserItems20130502* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT ui_id, charac_no, slot, it_id, expire_date, obtain_from, reg_date, ipg_agency_no, ability_no, stat, clear_avatar_id, item_lock_key, to_ipg_agency_no, m_time, hidden_option, emblem_endurance, color1, color2, trade_restrict FROM user_items_20130502");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_CAIN, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(UserItems20130502));
