#include "event_1107_avenger_plan_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int Event1107AvengerPlan_Add(DBConnectionManager* manager, const Event1107AvengerPlan* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO event_1107_avenger_plan (m_id, charac_no, occ_date) "
        "VALUES (%d, %d, '%s')",
...
int Event1107AvengerPlan_Get(DBConnectionManager* manager, int m_id, Event1107AvengerPlan* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT m_id, charac_no, occ_date FROM event_1107_avenger_plan WHERE m_id = %d",
        m_id);
    ...
    memset(record, 0, sizeof(Event1107AvengerPlan));
...
int Event1107AvengerPlan_GetAll(DBConnectionManager* manager, Event1107AvengerPlan* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT m_id, charac_no, occ_date FROM event_1107_avenger_plan");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_GAME_EVENT, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(Event1107AvengerPlan));
