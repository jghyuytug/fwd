#include "event_1112_ontime_info_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int Event1112OntimeInfo_Add(DBConnectionManager* manager, const Event1112OntimeInfo* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO event_1112_ontime_info (no, item_index, item_count, time) "
        "VALUES (%d, %d, %d, '%s')",
...
int Event1112OntimeInfo_Get(DBConnectionManager* manager, int no, Event1112OntimeInfo* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, item_index, item_count, time FROM event_1112_ontime_info WHERE no = %d",
        no);
    ...
    memset(record, 0, sizeof(Event1112OntimeInfo));
...
int Event1112OntimeInfo_GetAll(DBConnectionManager* manager, Event1112OntimeInfo* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT no, item_index, item_count, time FROM event_1112_ontime_info");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_SE_EVENT, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(Event1112OntimeInfo));
