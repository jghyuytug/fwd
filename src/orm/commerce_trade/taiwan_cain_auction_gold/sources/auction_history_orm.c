...
int AuctionHistory_Add(DBConnectionManager* manager, const AuctionHistory* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO auction_history (auction_id, start_time, occ_time, event_type, owner_id, buyer_id, price, seal_flag, item_id, add_info, upgrade, amplify_option, amplify_value, seal_cnt, endurance, extend_info, owner_postal_id, buyer_postal_id, unit_price) "
...
int AuctionHistory_Get(DBConnectionManager* manager, long long auction_id, AuctionHistory* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT auction_id, start_time, occ_time, event_type, owner_id, buyer_id, price, seal_flag, item_id, add_info, upgrade, amplify_option, amplify_value, seal_cnt, endurance, extend_info, owner_postal_id, buyer_postal_id, unit_price FROM auction_history WHERE auction_id = %d",
        auction_id);
...
int AuctionHistory_GetAll(DBConnectionManager* manager, AuctionHistory* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT auction_id, start_time, occ_time, event_type, owner_id, buyer_id, price, seal_flag, item_id, add_info, upgrade, amplify_option, amplify_value, seal_cnt, endurance, extend_info, owner_postal_id, buyer_postal_id, unit_price FROM auction_history");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_AUCTION_GOLD, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(AuctionHistory));
