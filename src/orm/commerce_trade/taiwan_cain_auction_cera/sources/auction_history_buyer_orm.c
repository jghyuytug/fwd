#include "auction_history_buyer_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int AuctionHistoryBuyer_Add(DBConnectionManager* manager, const AuctionHistoryBuyer* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO auction_history_buyer (auction_id, occ_time, pre_buyer_id, buyer_id, pre_price, price, pre_buyer_postal_id, commission) "
...
int AuctionHistoryBuyer_GetAll(DBConnectionManager* manager, AuctionHistoryBuyer* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT auction_id, occ_time, pre_buyer_id, buyer_id, pre_price, price, pre_buyer_postal_id, commission FROM auction_history_buyer");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_AUCTION_CERA, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(AuctionHistoryBuyer));
