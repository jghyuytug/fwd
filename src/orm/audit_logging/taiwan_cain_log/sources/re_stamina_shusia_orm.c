#include "re_stamina_shusia_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int ReStaminaShusia_Add(DBConnectionManager* manager, const ReStaminaShusia* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO re_stamina_shusia (occ_date, lev, occ_count) "
        "VALUES ('%s', %d, %d)",
...
int ReStaminaShusia_Get(DBConnectionManager* manager, const char* occ_date, signed char lev, ReStaminaShusia* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT occ_date, lev, occ_count FROM re_stamina_shusia WHERE occ_date = '%s' AND lev = %d",
        occ_date, lev);
    ...
    memset(record, 0, sizeof(ReStaminaShusia));
...
int ReStaminaShusia_GetAll(DBConnectionManager* manager, ReStaminaShusia* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT occ_date, lev, occ_count FROM re_stamina_shusia");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_CAIN_LOG, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(ReStaminaShusia));
