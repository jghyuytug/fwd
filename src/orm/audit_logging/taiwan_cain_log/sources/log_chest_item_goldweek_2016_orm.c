#include "log_chest_item_goldweek_2016_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int LogChestItemGoldweek2016_Add(DBConnectionManager* manager, const LogChestItemGoldweek2016* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO log_chest_item_goldweek_2016 (item_id, cnt) "
        "VALUES (%d, %d)",
...
int LogChestItemGoldweek2016_Get(DBConnectionManager* manager, int item_id, LogChestItemGoldweek2016* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT item_id, cnt FROM log_chest_item_goldweek_2016 WHERE item_id = %d",
        item_id);
    ...
    memset(record, 0, sizeof(LogChestItemGoldweek2016));
...
int LogChestItemGoldweek2016_GetAll(DBConnectionManager* manager, LogChestItemGoldweek2016* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT item_id, cnt FROM log_chest_item_goldweek_2016");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_CAIN_LOG, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(LogChestItemGoldweek2016));
