#include "dnf_party_result_history_pvp_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int DnfPartyResultHistoryPvp_Add(DBConnectionManager* manager, const DnfPartyResultHistoryPvp* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO dnf_party_result_history_pvp (str_ip, nat_type, m_id, party_result, occ_time, map_id, dungeon_id, peer_ip, peer_nat_type, established_time, IsConnectedToRelaySrv, ConnectedTimeToRelaySrv, relay_ip, relay_udp_port, relay_tcp_port) "
...
int DnfPartyResultHistoryPvp_Get(DBConnectionManager* manager, int id, DnfPartyResultHistoryPvp* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT id, str_ip, nat_type, m_id, party_result, occ_time, map_id, dungeon_id, peer_ip, peer_nat_type, established_time, IsConnectedToRelaySrv, ConnectedTimeToRelaySrv, relay_ip, relay_udp_port, relay_tcp_port FROM dnf_party_result_history_pvp WHERE id = %d",
        id);
    ...
    memset(record, 0, sizeof(DnfPartyResultHistoryPvp));
...
int DnfPartyResultHistoryPvp_GetAll(DBConnectionManager* manager, DnfPartyResultHistoryPvp* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT id, str_ip, nat_type, m_id, party_result, occ_time, map_id, dungeon_id, peer_ip, peer_nat_type, established_time, IsConnectedToRelaySrv, ConnectedTimeToRelaySrv, relay_ip, relay_udp_port, relay_tcp_port FROM dnf_party_result_history_pvp");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_CAIN_LOG, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(DnfPartyResultHistoryPvp));
