#include "channel_lev_status_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int ChannelLevStatus_Add(DBConnectionManager* manager, const ChannelLevStatus* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO channel_lev_status (occ_time, channel_no, lev, occ_count) "
        "VALUES ('%s', %d, %d, %d)",
        record->occ_time, record->channel_no, record->lev, record->occ_count);
...
int ChannelLevStatus_Get(DBConnectionManager* manager, const char* occ_time, signed char channel_no, signed char lev, ChannelLevStatus* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT occ_time, channel_no, lev, occ_count FROM channel_lev_status WHERE occ_time = '%s' AND channel_no = %d AND lev = %d",
    ...
    memset(record, 0, sizeof(ChannelLevStatus));
    ...
    record->occ_count = row[3] ? atoi(row[3]) : 0;
...
int ChannelLevStatus_GetAll(DBConnectionManager* manager, ChannelLevStatus* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT occ_time, channel_no, lev, occ_count FROM channel_lev_status");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_CAIN_LOG, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(ChannelLevStatus));
        ...
        records[count].occ_count = row[3] ? atoi(row[3]) : 0;
        count++;
    }
