#include "login_account_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#define MAX_QUERY_LEN 8192

int LoginAccount_Add(DBConnectionManager* manager, const LoginAccount* record) {
    char query[MAX_QUERY_LEN];
    DBQueryResult result;

    if (!manager || !record) return -1;

    snprintf(query, sizeof(query),
        "INSERT INTO login_account (m_id, m_channel_no, login_status, last_login_date, login_ip) "
        "VALUES (%d, %d, %d, '%s', '%s')",
        record->m_id, record->m_channel_no, record->login_status, record->last_login_date, record->login_ip);

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_CAIN_LOG, query, &result) < 0) {
        return -1;
    }

    DBQueryResult_Free(&result);
    return 0;
}

int LoginAccount_Get(DBConnectionManager* manager, int m_id, LoginAccount* record) {
    char query[MAX_QUERY_LEN];
    DBQueryResult result;
    char* row[5];

    if (!manager || !record) return -1;

    snprintf(query, sizeof(query),
        "SELECT m_id, m_channel_no, login_status, last_login_date, login_ip FROM login_account WHERE m_id = %d",
        m_id);

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_CAIN_LOG, query, &result) < 0)
        return -1;

    if (DBQueryResult_FetchRow(&result, row) <= 0) {
        DBQueryResult_Free(&result);
        return -1;
    }

    memset(record, 0, sizeof(LoginAccount));
    record->m_id = row[0] ? atoi(row[0]) : 0;
    record->m_channel_no = row[1] ? atoi(row[1]) : 0;
    record->login_status = row[2] ? atoi(row[2]) : 0;
    if (row[3]) strncpy(record->last_login_date, row[3], sizeof(record->last_login_date) - 1);
    if (row[4]) strncpy(record->login_ip, row[4], sizeof(record->login_ip) - 1);

    DBQueryResult_Free(&result);
    return 0;
}

int LoginAccount_GetAll(DBConnectionManager* manager, LoginAccount* records, int max_count, int* actual_count) {
    char query[MAX_QUERY_LEN];
    DBQueryResult result;
    char* row[5];
    int count = 0;

    if (!manager || !records || !actual_count) return -1;

    snprintf(query, sizeof(query),
        "SELECT m_id, m_channel_no, login_status, last_login_date, login_ip FROM login_account");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_CAIN_LOG, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(LoginAccount));
        records[count].m_id = row[0] ? atoi(row[0]) : 0;
        records[count].m_channel_no = row[1] ? atoi(row[1]) : 0;
        records[count].login_status = row[2] ? atoi(row[2]) : 0;
        if (row[3]) strncpy(records[count].last_login_date, row[3], sizeof(records[count].last_login_date) - 1);
        if (row[4]) strncpy(records[count].login_ip, row[4], sizeof(records[count].login_ip) - 1);
        count++;
    }

    *actual_count = count;
    DBQueryResult_Free(&result);
    return 0;
}
