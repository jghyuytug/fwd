#include "log_chest_item_goldweek_2013_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int LogChestItemGoldweek2013_Add(DBConnectionManager* manager, const LogChestItemGoldweek2013* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO log_chest_item_goldweek_2013 (item_id, cnt) "
        "VALUES (%d, %d)",
...
int LogChestItemGoldweek2013_Get(DBConnectionManager* manager, int item_id, LogChestItemGoldweek2013* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT item_id, cnt FROM log_chest_item_goldweek_2013 WHERE item_id = %d",
        item_id);
    ...
    memset(record, 0, sizeof(LogChestItemGoldweek2013));
...
int LogChestItemGoldweek2013_GetAll(DBConnectionManager* manager, LogChestItemGoldweek2013* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT item_id, cnt FROM log_chest_item_goldweek_2013");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_CAIN_LOG, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(LogChestItemGoldweek2013));
