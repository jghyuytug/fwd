...
int LogItemStat_Add(DBConnectionManager* manager, const LogItemStat* record) {
    ...
    snprintf(query, sizeof(query),
        "INSERT INTO log_item_stat (cur_date, server_info, process_id, item_id, item_cnt) "
        "VALUES ('%s', %d, %d, %d, %d)",
...
int LogItemStat_Get(DBConnectionManager* manager, const char* cur_date, signed char server_info, signed char process_id, int item_id, LogItemStat* record) {
    ...
    snprintf(query, sizeof(query),
        "SELECT cur_date, server_info, process_id, item_id, item_cnt FROM log_item_stat WHERE cur_date = '%s' AND server_info = %d AND process_id = %d AND item_id = %d",
    ...
    memset(record, 0, sizeof(LogItemStat));
...
int LogItemStat_GetAll(DBConnectionManager* manager, LogItemStat* records, int max_count, int* actual_count) {
    ...
    snprintf(query, sizeof(query),
        "SELECT cur_date, server_info, process_id, item_id, item_cnt FROM log_item_stat");

    if (DBConnectionManager_ExecuteQuery(manager, DB_TYPE_CAIN_LOG, query, &result) < 0)
        return -1;

    while (count < max_count && DBQueryResult_FetchRow(&result, row) > 0) {
        memset(&records[count], 0, sizeof(LogItemStat));
