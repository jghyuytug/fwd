...
def main():
    print("=" * 60)
    print("Key Expansion V17 - Target: 98%+ (2184 -> 2254+ keys)")
    print("=" * 60)
    st = PVFStringTable()
    ...
    new_mappings = {}
    # 取top 80来确保达到98%目标
    for key, stats in sorted_keys[:80]:
    ...
    with open('key_expansion_v20.py', 'w', encoding='utf-8') as f:
        f.write('#!/usr/bin/env python3\n"""V20 Keys - 98%+"""\n\nV20_KEYS = {\n')
        for k in sorted(new_mappings.keys()):
            f.write(f'    0x{k:04X}: "{new_mappings[k]}",\n')
        f.write("}\n")
    print("\nSaved to key_expansion_v20.py")
    print("\n# First 30 V20 keys:")
