...
def analyze(pvf, st, sample_size=200000):
    ...
    for entry in pvf.indexes:
        ...
        count += 1
        if count % 50000 == 0: print(f"  Processed {count} files...")
    return key_stats
...
def infer_semantic(key, stats, st):
    ...
    values = stats['values']
    if values:
        if all(0 <= v <= 1 for v in values): prefix += "bool_"
        elif all(0 <= v <= 100 for v in values): prefix += "pct_"
        elif all(1000 <= v <= 99999 for v in values): prefix += "id_"
    return f"{prefix}{type_map.get(main_type, 'field')}_{key:04x}"

def main():
    print("=" * 60)
    print("Key Expansion V11 - Target: 70%+ (1508 -> 1610+ keys)")
    print("=" * 60)
    st = PVFStringTable()
    st.load_from_file('C:/Users/waw/Desktop/game/stringtable.bin')
    pvf = PVFReader('C:/Users/waw/Desktop/新建文件夹 (2)/Script.pvf')
    pvf.open()
    pvf.parse_index(max_entries=210000)
    ...
    new_mappings = {}
    for key, stats in sorted_keys[:110]:
    ...
    with open('key_expansion_v14.py', 'w', encoding='utf-8') as f:
        f.write('#!/usr/bin/env python3\n"""V14 Keys - 70%+"""\n\nV14_KEYS = {\n')
        for k in sorted(new_mappings.keys()):
            f.write(f'    0x{k:04X}: "{new_mappings[k]}",\n')
        f.write("}\n")
    print("\nSaved to key_expansion_v14.py")
    print("\n# First 30 V14 keys:")
