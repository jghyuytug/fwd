...
def analyze(pvf, st, sample_size=140000):
    key_stats = defaultdict(lambda: {'count': 0, 'types': Counter(), 'formats': set()})
    count = 0
    for entry in pvf.indexes:
        ...
        count += 1
        if count % 35000 == 0: print(f"  Processed {count} files...")
    return key_stats
...
def main():
    print("=" * 60)
    print("Key Expansion V8 - Target: 55%+ (1156 -> 1265+ keys)")
    print("=" * 60)
    ...
    for key, stats in sorted_keys[:115]:
        ...
        if not semantic:
            ...
            prefix = (list(stats['formats'])[0][1:] + "_") if len(stats['formats']) == 1 else ""
            semantic = f"{prefix}{type_map.get(main_type, 'field')}_{key:04x}"
        if semantic: new_mappings[key] = semantic

    total = len(KNOWN_KEYS) + len(new_mappings)
    print(f"\nNew: {len(new_mappings)}, Total: {total}, Coverage: {total/2300*100:.1f}%")

    with open('key_expansion_v11.py', 'w', encoding='utf-8') as f:
        f.write('#!/usr/bin/env python3\n"""V11 Keys - 55%+"""\n\nV11_KEYS = {\n')
        for k in sorted(new_mappings.keys()): f.write(f'    0x{k:04X}: "{new_mappings[k]}",\n')
        f.write("}\n")

    print("\nSaved to key_expansion_v11.py")
    print("\n# First 30 V11 keys:")
    for k in sorted(new_mappings.keys())[:30]: print(f'    0x{k:04X}: "{new_mappings[k]}",')
