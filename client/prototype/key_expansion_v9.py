...
def analyze(pvf, st, sample_size=160000):
    ...
    for entry in pvf.indexes:
        ...
        count += 1
        if count % 40000 == 0: print(f"  Processed {count} files...")
    return key_stats
...
def main():
    ...
    with open('key_expansion_v12.py', 'w', encoding='utf-8') as f:
        f.write('#!/usr/bin/env python3\n"""V12 Keys - 60%+"""\n\nV12_KEYS = {\n')
        for k in sorted(new_mappings.keys()):
            f.write(f'    0x{k:04X}: "{new_mappings[k]}",\n')
        f.write("}\n")
    print("\nSaved to key_expansion_v12.py")
    print("\n# First 30 V12 keys:")
