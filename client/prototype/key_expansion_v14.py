...
def analyze(pvf, st, sample_size=260000):
...
def main():
    print("=" * 60)
    print("Key Expansion V14 - Target: 85%+ (1841 -> 1955+ keys)")
    print("=" * 60)
    st = PVFStringTable()
    ...
    with open('key_expansion_v17.py', 'w', encoding='utf-8') as f:
        f.write('#!/usr/bin/env python3\n"""V17 Keys - 85%+"""\n\nV17_KEYS = {\n')
        for k in sorted(new_mappings.keys()):
            f.write(f'    0x{k:04X}: "{new_mappings[k]}",\n')
        f.write("}\n")
    print("\nSaved to key_expansion_v17.py")
    print("\n# First 30 V17 keys:")
