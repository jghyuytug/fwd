...
def analyze(pvf, st, sample_size=180000):
    ...
    for entry in pvf.indexes:
        ...
        count += 1
        if count % 45000 == 0: print(f"  Processed {count} files...")
    return key_stats
...
def main():
    ...
    with open('key_expansion_v13.py', 'w', encoding='utf-8') as f:
        f.write('#!/usr/bin/env python3\n"""V13 Keys - 65%+"""\n\nV13_KEYS = {\n')
        for k in sorted(new_mappings.keys()):
            f.write(f'    0x{k:04X}: "{new_mappings[k]}",\n')
        f.write("}\n")
    print("\nSaved to key_expansion_v13.py")
    print("\n# First 30 V13 keys:")
