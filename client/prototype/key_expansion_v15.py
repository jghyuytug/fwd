...
def main():
    print("=" * 60)
    print("Key Expansion V15 - Target: 90%+ (1956 -> 2070+ keys)")
    print("=" * 60)
    st = PVFStringTable()
    ...
    with open('key_expansion_v18.py', 'w', encoding='utf-8') as f:
        f.write('#!/usr/bin/env python3\n"""V18 Keys - 90%+"""\n\nV18_KEYS = {\n')
        for k in sorted(new_mappings.keys()):
            f.write(f'    0x{k:04X}: "{new_mappings[k]}",\n')
        f.write("}\n")
    print("\nSaved to key_expansion_v18.py")
    print("\n# First 30 V18 keys:")
