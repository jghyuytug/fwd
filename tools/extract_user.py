...
"""
核心业务代码提取工具
从 df_game_r_v3.c 中提取 CUser、CUserCharacInfo、CInventory、Inven_Item 相关代码
"""
...
class UserExtractor:
    def __init__(self, source_file):
        ...
        # 目标类（分为两组）
        self.user_classes = ['CUser', 'CUserCharacInfo']
        self.inventory_classes = ['CInventory', 'Inven_Item']
        self.target_classes = self.user_classes + self.inventory_classes
    ...
    def get_output_dir(self, class_name):
        """根据类名返回输出目录"""
        if class_name in self.user_classes:
            return Path("src/user")
        else:
            return Path("src/inventory")

    def get_include_dir(self, class_name):
        """根据类名返回头文件目录"""
        if class_name in self.user_classes:
            return Path("include/user")
        else:
            return Path("include/inventory")
    ...
    def generate_files(self):
        ...
        # 创建目录
        for cls in self.target_classes:
            output_dir = self.get_output_dir(cls)
            include_dir = self.get_include_dir(cls)
    ...
    def generate_header(self, class_name):
        """生成头文件"""
        include_dir = self.get_include_dir(class_name)
        header_file = include_dir / f"{class_name}.h"
        ...
        with open(header_file, 'w', encoding='utf-8') as f:
            ...
            if class_name in ['CUser', 'CUserCharacInfo']:
                f.write("#include <common/constants_improved.h>\n")
                f.write("#include <common/error_codes.h>\n")
            if class_name in ['CInventory', 'Inven_Item']:
                f.write("#include <user/CUser.h>\n")
            f.write("\n")
    ...
    def generate_source(self, class_name):
        ...
        output_dir = self.get_output_dir(class_name)
        source_file = output_dir / f"{class_name}.cpp"
        ...
        with open(source_file, 'w', encoding='utf-8') as f:
            # 包含头文件
            if class_name in self.user_classes:
                f.write(f"#include <user/{class_name}.h>\n")
            else:
                f.write(f"#include <inventory/{class_name}.h>\n")
    ...
    def print_statistics(self):
        ...
        for cls in self.user_classes:
            ...
            if decl_count > 0 or impl_count > 0:
                ...
                total_user_decl += decl_count
                total_user_impl += impl_count

        print(f"\n库存管理模块:")
        total_inv_decl = 0
        total_inv_impl = 0
        for cls in self.inventory_classes:
            decl_count = len(self.declarations[cls])
            impl_count = len(self.implementations[cls])
            if decl_count > 0 or impl_count > 0:
                print(f"  {cls}:")
                print(f"    声明: {decl_count:4d}")
                print(f"    实现: {impl_count:4d}")
                total_inv_decl += decl_count
                total_inv_impl += impl_count

        print(f"\n总计:")
        print(f"  用户模块:")
        print(f"    声明: {total_user_decl:4d}")
        print(f"    实现: {total_user_impl:4d}")
        print(f"  库存模块:")
        print(f"    声明: {total_inv_decl:4d}")
        print(f"    实现: {total_inv_impl:4d}")
        print(f"  整体:")
        print(f"    声明: {total_user_decl + total_inv_decl:4d}")
        print(f"    实现: {total_user_impl + total_inv_impl:4d}")
        print(f"    结构体: {len(self.structures):4d}")

        print(f"\n输出目录:")
        print(f"  用户模块: src/user/, include/user/")
        print(f"  库存模块: src/inventory/, include/inventory/")
        print(f"{'='*60}\n")
...
def main():
    ...
    extractor = UserExtractor(source_file)
    extractor.extract()
