...
int main(int argc, char* argv[]) {
    ...
    printf("=== Member Game Option ORM Test Suite ===\n\n");
...
void cleanup_test_data(DBConnectionManager* manager) {
    ...
    snprintf(query, sizeof(query),
        "DELETE FROM member_game_option WHERE m_id IN (200001, 200002, 200003)");
...
void test_basic_crud(DBConnectionManager* manager) {
    MemberGameOption option, retrieved;
    int ret;
    ...
    ret = MemberGameOption_Add(manager, &option);
    if (ret == 0) {
    ...
    /* Test 2: Get option */
    test_total++;
    MemberGameOption_Init(&retrieved);
    ret = MemberGameOption_Get(manager, 200001, &retrieved);
    if (ret == 0 && retrieved.m_id == 200001) {
        printf("[PASS] Test 2: Get option\n");
        test_passed++;
        MemberGameOption_PrintInfo(&retrieved);
    } else {
        printf("[FAIL] Test 2: Get option (returned %d)\n", ret);
    }
    MemberGameOption_Free(&retrieved);

    /* Test 3: Exists check */
    test_total++;
    ret = MemberGameOption_Exists(manager, 200001);
    if (ret > 0) {
    ...
    test_total++;
    ret = MemberGameOption_Update(manager, &option);
    if (ret == 0) {
    ...
    test_total++;
    ret = MemberGameOption_Delete(manager, 200001);
    if (ret == 0) {
    ...
    MemberGameOption_Init(&option);
    option.m_id = 200002;
    MemberGameOption_Add(manager, &option);

    option.m_id = 200003;
    MemberGameOption_Add(manager, &option);

    ret = MemberGameOption_Exists(manager, 200002);
    if (ret > 0 && MemberGameOption_Exists(manager, 200003) > 0) {
