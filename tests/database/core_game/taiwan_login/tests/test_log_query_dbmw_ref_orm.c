...
int main(int argc, char* argv[]) {
    ...
    printf("=== Log Query DBMW Ref ORM Test Suite ===\n\n");
...
void cleanup_test_data(DBConnectionManager* manager) {
    ...
    snprintf(query, sizeof(query),
        "DELETE FROM log_query_dbmw_ref WHERE query_hash LIKE 'testhash%%' OR query_hash LIKE 'uniquehash%%'");
...
void test_basic_crud(DBConnectionManager* manager) {
    LogQueryDbmwRef record1, record2, fetched;
    int exists;

    printf("TEST 1: Add first query reference (AUTO_INCREMENT)\n");
    test_total++;
    memset(&record1, 0, sizeof(LogQueryDbmwRef));
    ...
    if (LogQueryDbmwRef_Add(manager, &record1) == 0) {
        printf("  PASS\n");
        test_passed++;
    } else {
    ...
    test_total++;
    memset(&record2, 0, sizeof(LogQueryDbmwRef));
    ...
    if (LogQueryDbmwRef_Add(manager, &record2) == 0) {
        printf("  PASS\n");
        test_passed++;
    } else {
        printf("  FAIL: Failed to add record\n");
    ...
    test_total++;
    memset(&fetched, 0, sizeof(LogQueryDbmwRef));
    if (LogQueryDbmwRef_GetByHash(manager, "testhash001", &fetched) == 0) {
    ...
    memset(&fetched, 0, sizeof(LogQueryDbmwRef));
    if (LogQueryDbmwRef_Get(manager, q_id_to_fetch, &fetched) == 0) {
    ...
    test_total++;
    exists = LogQueryDbmwRef_Exists(manager, q_id_to_fetch);
    if (exists == 1) {
        printf("  PASS\n");
        test_passed++;
    } else {
    ...
    test_total++;
    exists = LogQueryDbmwRef_HashExists(manager, "testhash001");
    if (exists == 1) {
        printf("  PASS\n");
        test_passed++;
    } else {
    ...
    if (LogQueryDbmwRef_Update(manager, &fetched) == 0) {
        memset(&fetched, 0, sizeof(LogQueryDbmwRef));
        if (LogQueryDbmwRef_Get(manager, q_id_to_fetch, &fetched) == 0) {
    ...
    test_total++;
    if (LogQueryDbmwRef_Delete(manager, q_id_to_fetch) == 0) {
        exists = LogQueryDbmwRef_Exists(manager, q_id_to_fetch);
        if (exists == 0) {
            printf("  PASS\n");
            test_passed++;
    ...
    test_total++;
    if (LogQueryDbmwRef_DeleteByHash(manager, "testhash002") == 0) {
        exists = LogQueryDbmwRef_HashExists(manager, "testhash002");
        if (exists == 0) {
            printf("  PASS\n");
            test_passed++;
        } else {
...
void test_unique_constraint(DBConnectionManager* manager) {
    ...
    if (LogQueryDbmwRef_Add(manager, &record) == 0) {
        printf("  PASS\n");
        test_passed++;
    } else {
        printf("  FAIL: Failed to add record\n");
...
void test_business_operations(DBConnectionManager* manager) {
    LogQueryDbmwRef records[10];
    int actual_count;
    int total_count;

    /* Add test data */
    LogQueryDbmwRef test_records[3];
    ...
    LogQueryDbmwRef_Add(manager, &test_records[0]);
    LogQueryDbmwRef_Add(manager, &test_records[1]);
    LogQueryDbmwRef_Add(manager, &test_records[2]);

    printf("TEST 12: Get all query references\n");
    test_total++;
    actual_count = 0;
    if (LogQueryDbmwRef_GetAll(manager, records, 10, &actual_count) == 0) {
    ...
    test_total++;
    LogQueryDbmwRef latest;
    if (LogQueryDbmwRef_GetLatest(manager, &latest) == 0) {
    ...
    test_total++;
    total_count = LogQueryDbmwRef_Count(manager);
    if (total_count >= 3) {
    ...
    test_total++;
    if (actual_count > 0) {
        LogQueryDbmwRef_PrintInfo(&records[0]);
        printf("  PASS\n");
        test_passed++;
    } else {
