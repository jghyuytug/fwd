#include "member_play_info_20130502_orm.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void cleanup_test_data(DBConnectionManager* manager) {
    ...
    snprintf(query, sizeof(query),
        "DELETE FROM member_play_info_20130502 WHERE m_id IN (100001, 100002, 100003)");
...
void test_basic_crud(DBConnectionManager* manager) {
    ...
    memset(&info, 0, sizeof(MemberPlayInfo20130502));
    ...
    ret = MemberPlayInfo20130502_Add(manager, &info);
    check_test("Add new record", ret == 0);

    /* Test 2: Check record exists */
    ret = MemberPlayInfo20130502_Exists(manager, "2025-01-15", 100001);
    check_test("Record exists", ret == 1);

    /* Test 3: Get record */
    MemberPlayInfo20130502 retrieved;
    ret = MemberPlayInfo20130502_Get(manager, "2025-01-15", 100001, &retrieved);
    ...
    retrieved.exp = 2000;
    ret = MemberPlayInfo20130502_Update(manager, &retrieved);
    check_test("Update record", ret == 0);

    /* Test 5: Verify update */
    MemberPlayInfo20130502 updated;
    ret = MemberPlayInfo20130502_Get(manager, "2025-01-15", 100001, &updated);
    ...
    ret = MemberPlayInfo20130502_Delete(manager, "2025-01-15", 100001);
...
void test_business_operations(DBConnectionManager* manager) {
    MemberPlayInfo20130502 info;
    MemberPlayInfo20130502 results[10];
    int actual_count;
    int ret;
    ...
    info.server_id = 1;
    info.pcbang_flag = 0;
    MemberPlayInfo20130502_Add(manager, &info);
    ...
    ret = MemberPlayInfo20130502_GetByMemberId(manager, 100002, results, 10, &actual_count);
    check_test("Get by member ID", ret == 0 && actual_count == 3);

    /* Test 2: Get by date range */
    ret = MemberPlayInfo20130502_GetByDateRange(manager, "2025-01-15", "2025-01-16",
                                        results, 10, &actual_count);
    check_test("Get by date range", ret == 0 && actual_count >= 3);

    /* Test 3: Get by server */
    ret = MemberPlayInfo20130502_GetByServer(manager, 1, results, 10, &actual_count);
    check_test("Get by server", ret == 0 && actual_count >= 4);

    /* Test 4: Get PC bang users */
    ret = MemberPlayInfo20130502_GetPCBangUsers(manager, "2025-01-15", results, 10, &actual_count);
...
int main(int argc, char* argv[]) {
    ...
    printf("=== Member Play Info 20130502 ORM Test Suite ===\n");
