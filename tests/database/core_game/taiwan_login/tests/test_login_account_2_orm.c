...
int main(int argc, char* argv[]) {
    ...
    printf("=== Login Account 2 ORM Test Suite ===\n\n");
...
void cleanup_test_data(DBConnectionManager* manager) {
    ...
    snprintf(query, sizeof(query),
        "DELETE FROM login_account_2 WHERE m_id IN (100001, 100002, 100003)");
    DBConnectionManager_ExecuteQuery(manager, DB_TYPE_LOGIN, query, &result);
    DBQueryResult_Free(&result);
}

void test_basic_crud(DBConnectionManager* manager) {
    LoginAccount2 record;
    int exists;

    printf("Test 1: Add login account\n");
    test_total++;
    memset(&record, 0, sizeof(LoginAccount2));
    record.m_id = 100001;
    ...
    if (LoginAccount2_Add(manager, &record) == 0) {
    ...
    test_total++;
    exists = LoginAccount2_Exists(manager, 100001);
    if (exists) {
    ...
    test_total++;
    memset(&record, 0, sizeof(LoginAccount2));
    if (LoginAccount2_Get(manager, 100001, &record) == 0) {
    ...
    if (LoginAccount2_Update(manager, &record) == 0) {
        memset(&record, 0, sizeof(LoginAccount2));
        LoginAccount2_Get(manager, 100001, &record);
    ...
    test_total++;
    if (LoginAccount2_Delete(manager, 100001) == 0) {
        exists = LoginAccount2_Exists(manager, 100001);
        if (!exists) {
...
void test_business_operations(DBConnectionManager* manager) {
    ...
    strcpy(record.last_login_date, "2025-11-19 16:00:00");
    strcpy(record.login_ip, "192.168.1.100");
    LoginAccount2_Add(manager, &record);
    ...
    test_total++;
    if (LoginAccount2_GetByStatus(manager, 1, records, 10, &actual_count) == 0) {
        if (actual_count >= 2) {
    ...
    test_total++;
    if (LoginAccount2_GetByChannel(manager, 1, records, 10, &actual_count) == 0) {
        if (actual_count >= 2) {
    ...
    test_total++;
    if (LoginAccount2_UpdateLoginStatus(manager, 100001, 0) == 0) {
        memset(&record, 0, sizeof(LoginAccount2));
        LoginAccount2_Get(manager, 100001, &record);
    ...
    test_total++;
    if (LoginAccount2_UpdateLastLogin(manager, 100001, "2025-11-19 17:00:00", "192.168.1.200") == 0) {
        memset(&record, 0, sizeof(LoginAccount2));
        LoginAccount2_Get(manager, 100001, &record);
