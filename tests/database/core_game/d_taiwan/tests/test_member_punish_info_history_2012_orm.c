...
int main(int argc, char* argv[]) {
    ...
    printf("=== Member Punish Info History 2012 ORM Test Suite ===\n\n");
...
void test_basic_crud(DBConnectionManager* manager) {
    ...
    MemberPunishInfoHistory2012 fetched;
    ...
    strcpy(record.occ_time, "2012-01-15 10:30:00");
    record.punish_value = 7;
    record.apply_flag = 1;
    strcpy(record.start_time, "2012-01-15 10:30:00");
    strcpy(record.end_time, "2012-01-22 10:30:00");
    strcpy(record.admin_id, "admin001");
    strcpy(record.reason, "Illegal game behavior");
    record.is_kicked = 1;

    if (MemberPunishInfoHistory2012_Add(manager, &record) < 0) {
    ...
    /* Count records */
    int count = MemberPunishInfoHistory2012_CountByMId(manager, 100001);
    if (count < 1) {
    ...
    MemberPunishInfoHistory2012 records[10];
    int actual_count = 0;
    if (MemberPunishInfoHistory2012_GetByMId(manager, 100001, records, 10, &actual_count) < 0 || actual_count < 1) {
    ...
    if (MemberPunishInfoHistory2012_Get(manager, test_no, &fetched) < 0) {
    ...
    if (MemberPunishInfoHistory2012_Update(manager, &fetched) < 0) {
    ...
    /* Test Delete */
    if (MemberPunishInfoHistory2012_Delete(manager, test_no) < 0) {
...
void test_query_operations(DBConnectionManager* manager) {
    ...
    for (int i = 0; i < 3; i++) {
        MemberPunishInfoHistory2012 rec;
        memset(&rec, 0, sizeof(MemberPunishInfoHistory2012));
        rec.m_id = 200001;
        rec.punish_type = i + 1;  /* Types: 1, 2, 3 */
        snprintf(rec.occ_time, sizeof(rec.occ_time), "2012-06-%02d 14:00:00", 10 + i);
        rec.punish_value = (i + 1) * 3;
        rec.apply_flag = (i == 2) ? 0 : 1;
        snprintf(rec.start_time, sizeof(rec.start_time), "2012-06-%02d 14:00:00", 10 + i);
        snprintf(rec.end_time, sizeof(rec.end_time), "2012-06-%02d 14:00:00", 17 + i);
        snprintf(rec.admin_id, sizeof(rec.admin_id), "admin%03d", 100 + i);
        snprintf(rec.reason, sizeof(rec.reason), "Test reason %d", i + 1);
        rec.is_kicked = (i % 2);

        if (MemberPunishInfoHistory2012_Add(manager, &rec) < 0) {
    ...
    /* Test GetByMId */
    if (MemberPunishInfoHistory2012_GetByMId(manager, 200001, records, 10, &actual_count) < 0) {
    ...
    if (MemberPunishInfoHistory2012_GetByPunishType(manager, 2, records, 10, &actual_count) < 0) {
    ...
    /* Test GetByTimeRange - only first record (2012-06-10) */
    if (MemberPunishInfoHistory2012_GetByTimeRange(manager, "2012-06-10 00:00:00", "2012-06-10 23:59:59",
    ...
    if (MemberPunishInfoHistory2012_UpdateApplyFlag(manager, first_no, 0) < 0) {
    ...
    /* Verify flag update */
    MemberPunishInfoHistory2012 updated;
    if (MemberPunishInfoHistory2012_Get(manager, first_no, &updated) < 0) {
...
void cleanup_test_data(DBConnectionManager* manager) {
    ...
    snprintf(query, sizeof(query),
        "DELETE FROM member_punish_info_history_2012 WHERE m_id IN (100001, 200001)");
